...
def make_torrent_file(input, userabortflag = None, userprogresscallback = lambda x: None):
    ...
    if 'creation date' not in input:
        cdate = long(time())
    else:
        cdate = long(input['creation date'])
    metainfo = {'info': info, 'encoding': input['encoding'], 'creation date': cdate}
    ...
    # Arno, 2011-01-28: Make torrents more compact, use 'playtime'
    # Arno, 2011-03-02: Temporary reinstated, PDD uses SpeedBps directly
    # apparently.
    #
    ...
    # Arno, 2011-09-12: I need the ability to save them in a .tstream for
    # peer-assisted download.
    #
    if 'initial peers' in input:
        metainfo['initial peers'] = input['initial peers']

    if 'predef seeders' in input:
        metainfo['predef seeders'] = input['predef seeders']
...
def copy_metainfo_to_input(metainfo,input):
    ...
    if 'creation date' in metainfo:
        input['creation date'] = metainfo['creation date']
