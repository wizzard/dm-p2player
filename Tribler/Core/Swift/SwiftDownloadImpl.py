    ...
    def network_get_state(self,usercallback,getpeerlist,sessioncalling=False):
        ...
        try:
            if self.sp is None:
                ...
                (status,stats,logmsgs,coopdl_helpers,coopdl_coordinator) = self.network_get_stats(getpeerlist)
                ds = DownloadState(self,status,self.error,self.get_progress(),stats=stats,logmsgs=logmsgs,coopdl_helpers=coopdl_helpers,coopdl_coordinator=coopdl_coordinator)
