...
if sys.frozen == "windows_exe":
    class Stderr(object):
        ...
        def open_file(self):
            try:
                s0 = os.stat(self._fname)
                if s0.st_size > self._limit:
                    # Delete .1, rename
                    try:
                        os.remove(self.other_fname())
                    except:
                        pass
                    os.rename(self._fname,self.other_fname())
            except:
                pass
            self._file = open(self._fname,"a")
        
        def other_fname(self):
            return self._fname+'.1'
