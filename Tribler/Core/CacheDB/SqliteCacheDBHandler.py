...
#
# Arno: WARNING!
# The sqlite database cannot handle binary data, so all binary data (PermIDs, 
# infohashes, signatures) has to be BASE64 encoding before being written.
#
# MORE IMPORTANTLY HOWEVER: this anomaly must not be visible outside this
# DBHandler. That is, all methods must take binary arguments and return results
# with the appropriate fields converted back to binary!
#
#
import sys
import os
...
# Arno: only place where this is allowed:
from Tribler.Core.CacheDB.sqlitecachedb import SQLiteCacheDB, bin2str, str2bin, NULL
from Tribler.Core.CacheDB.RichMetadataSearchGrammar import generateSQLQuery
...
class SuperPeerDBHandler(BasicDBHandler):
    ...
    def loadSuperPeers(self, config, refresh=False):
        ...
        print >>sys.stderr,"Superpeer list",`superpeer_list`
...
class TorrentDBHandler(BasicDBHandler):
    ...
    def _addTorrentToDB(self, torrentdef, source, extra_info, commit):
        ...
        # UNIKLU: extract rich metadata from torrent and store it in the 
        # Richmetadata table
        try:
            self._addRichMetadataForTorrent(torrent_id, torrentdef, commit=False)
        except:
            print_exc()
    ...
    def deleteTorrent(self, infohash, delete_file=False, commit = True):
        ...
        if deleted:
            # Arno, 2011-06-16: What's this keep_infohash? Now: really delete!
            self._deleteTorrent(infohash, keep_infohash=not delete_file, commit=commit)
        ...
        if torrent_id is not None:
            if keep_infohash:
                ...
                try:
                    ...
                    print_exc()
                try:
                    # Arno, 2011-06-16: NextShare XML Rich Metadata
                    nsrichmetadata_db = RichMetadataDBHandler()
                    nsrichmetadata_db.deleteTorrentByID(torrent_id,commit=commit)
                except:
                    print_exc()
...
#            torrents2del = 100
    ...
    def _addRichMetadataForTorrent(self, torrent_id, torrentdef, commit=True):
        assert isinstance(torrentdef, TorrentDef), "TORRENTDEF has invalid type: %s" % type(torrentdef)
        assert torrentdef.is_finalized(), "TORRENTDEF is not finalized"        
        metadata = torrentdef.get_metadata()
        fields_dict = dict()
        if metadata:
            from JSI.RichMetadata.RichMetadata import RichMetadataGenerator,RichMetadata
            rmm = RichMetadataGenerator.getInstance()
            richmeta = rmm.getRichMetadata(metadata)
            metacore = rmm.getRichMetadata(richmeta.getMetaCore())
            for mn in metacore.getAPIMethods():
                # call all getter methods of the API to get all rich metadata
                if mn.startswith("get"):
                    fieldname = mn[3:].lower()
                    result = getattr(metacore,mn)()
                    if result is not None:
                        # torrent stores duration as iso8601 string, database in seconds
                        if fieldname == 'duration':
                            fields_dict[fieldname] = iso8601_duration2seconds(result)
                        else:
                            fields_dict[fieldname] = result
            db_dict = self._get_richmeta_db_dict(torrent_id, fields_dict)
            self._db.insert("RichMetadata", commit=commit, **db_dict)
       

    # determines where (in which columns of the rich metadata table)
    # to store the given metadata values.
    def _get_richmeta_db_dict(self, torrent_id, metadata_dict):
        db_dict = dict() 
        db_dict["torrent_id"] = torrent_id
        optdbfields = { 
            ...
            "publisher":"publisher",
            "relatedmaterial" : "related_content",      
            "releasedate":"release_date",       
            "releaseinformation":"release_information",
            "signlanguage":"sign_language",      
            "synopsis":"synopsis",          
            "titleepisodetitle":"title_episode_title",
            "titlemain":"title_main",         
            "titleseriestitle":"title_series_title",  
            "verticalsize":"vertical_size",      
            "videocoding":"video_coding",
        }
        
        for field in optdbfields.keys():
            fieldval = metadata_dict.get(field) 
            if fieldval is not None:
                db_dict[optdbfields[field]] = fieldval
...
class VoteCastDBHandler(BasicDBHandler):
    ...
    def _strvote2votetup(self,strvote):
        return (str2bin(strvote[0]),str2bin(strvote[1]),strvote[2],strvote[3])

    def _strvotes2votetuples(self,strvotes):
        votetups = []
        for strvote in strvotes:
            votetup = self._strvote2votetup(strvote)
            votetups.append(votetup)
        return votetups

    def addVote(self,mod_id,voter_id,vote,time_stamp):
        """ 
        Add vote "vote" for "mod_id" by "voter_id" made at "time_stamp"
        @param mod_id PermID
        @param voter_id PermID
        @param vote int
        @param time_stamp int
        """
        if self.getVote(mod_id,voter_id) is not None:
            self.deleteVote(mod_id,voter_id)
            
        strvotedict = {'mod_id':bin2str(mod_id),'voter_id':bin2str(voter_id),'vote':vote,'time_stamp':time_stamp}
        self._db.insert(self.table_name, **strvotedict)        

    def getVote(self,mod_id,voter_id):
        """ 
        ...
        return self._db.fetchone(sql, (bin2str(mod_id),bin2str(voter_id),))    

    def deleteVote(self,mod_id,voter_id):
        """ 
        Delete vote for "mod_id" by "voter_id"
        @param mod_id PermID
        @param voter_id PermID
        """
        sql = 'Delete From VoteCast where mod_id==? and voter_id==?'
        self._db.execute_write(sql,(bin2str(mod_id),bin2str(voter_id),))

    def deleteVotes(self, mod_id):
        """ 
        Delete votes for "mod_id"
        @param mod_id PermID
        @param voter_id PermID
        """
        sql = 'Delete From VoteCast where mod_id==?'
        self._db.execute_write(sql,(bin2str(mod_id),))
    
    def getVotes(self,mod_id):
        """ 
        Return all votes for moderator "mod_id" as list of tuples
        @param mod_id PermID
        @return [(mod_id,voter_id,vote,timestamp)]
        """
        sql = 'select * from VoteCast where mod_id==?'
        
        strvotes = self._db.fetchall(sql, (bin2str(mod_id),))
        return self._strvotes2votetuples(strvotes)
    
    def getAllVotes(self):
        """ 
        Return all votes as list of tuples
        @return [(mod_id,voter_id,vote,timestamp)]
        """
        sql = 'select * from VoteCast'
        
        strvotes = self._db.fetchall(sql)
        return self._strvotes2votetuples(strvotes)
        
    def getMyRecentAndRandomVotes(self, recent=25, random=25):
        """ 
        Return selection of my votes as list of tuples
        @param recent int
        @param random int
        @return [(mod_id,voter_id,vote,timestamp)]
        """
        ...
        myrecentvotes = self._db.fetchall(sql,(bin2str(self.my_permid),recent,))
        ...
        if myrecentvotes is not None and len(myrecentvotes)>=recent:
            ...
            myrandomvotes = self._db.fetchall(sql,(bin2str(self.my_permid),t,random,))
    ...
    #
    # Convenience methods for my_permid
    #
    def subscribe(self,mod_id):
        ...
        """
        sql = "select vote from VoteCast where mod_id==? and voter_id==?"
        vote = self._db.fetchone(sql,(bin2str(mod_id),bin2str(self.my_permid),))
        if vote is None:
            sql = "insert into VoteCast Values(?,?,'2',?)"
            self._db.execute_write(sql,(bin2str(mod_id),bin2str(self.my_permid),now(),))
        elif vote!=2:
            sql = "update VoteCast set vote=2 where mod_id==? and voter_id==?"
            self._db.execute_write(sql,(bin2str(mod_id),bin2str(self.my_permid),))    

        self.notifier.notify(NTFY_CHANNELCAST, NTFY_UPDATE, mod_id)   
    ...
    def getMySubscriptions(self):
        """
        Returns mod_ids to which I am subscribed
        @return list of PermIDs 
        """
        sql = "select mod_id from VoteCast where voter_id==? and vote=2"
        subs = []
        records = self._db.fetchall(sql, (bin2str(self.my_permid),))
        for record in records:
            subs.append(str2bin(record[0]))
        return subs
        
        self.notifier.notify(NTFY_CHANNELCAST, NTFY_UPDATE, permid)
    
    def spam(self, mod_id):
        """ 
    ...
    def getModIdsWithNegVote(self, voter_id):
        """ 
        Return the mod_ids having a negative vote from voter_id
        @return list of PermIDs 
        """
        sql = "select mod_id from VoteCast where voter_id==? and vote=-1"
        res = self._db.fetchall(sql,(bin2str(voter_id),))
        result_list = []
        for entry in res:
            result_list.append(str2bin(entry[0]))
        return result_list

    def getNumNegVotes(self,mod_id):
        """
        Returns the number of negative votes
        @return int
        """
        sql = "select count(*) from VoteCast where mod_id==? and vote=-1"
        return self._db.fetchone(sql, (bin2str(mod_id),))
    
    def getNumSubscriptions(self,mod_id): ###
        """
...
class ChannelCastDBHandler(BasicDBHandler):
    ...
    channelcast_db = session.open_dbhandler(NTFY_CHANNELCAST)
    channelcast_db.addOwnTorrent(tdef)
    
    Get Available Channels
    ======================    
    channelcast_db = session.open_dbhandler(NTFY_CHANNELCAST)
    tuplelist = channelcast_db.getOtherChannels()
    # tuple = [(permid,channel_name,#votes)]
    ...
    def addOwnTorrent(self, torrentdef):
        ...
        @param torrentdef TorrentDef
        """
        assert isinstance(torrentdef, TorrentDef), "TORRENTDEF has invalid type: %s" % type(torrentdef)
        flag = False
        spublisher_id = bin2str(self.my_permid)
        sinfohash = bin2str(torrentdef.get_infohash())
        sql = "select count(*) from ChannelCast where publisher_id==? and infohash==?"
        num_records = self._db.fetchone(sql, (spublisher_id, sinfohash,))
        if num_records==0:
            storrenthash = bin2str(sha(bencode(torrentdef.get_metainfo())).digest())
            ...
            record = [spublisher_id,unickname,sinfohash,storrenthash,utorrentname,now()]
            self._sign(record)
    ...
    def deleteOwnTorrent(self, infohash):
        """
        Remove a torrent from your channel
        @param infohash 20-byte string
        """
    ...
    def deleteTorrentsFromPublisherId(self, publisher_id):
        """
        Remove info about torrents published in channel "publisher_id". Note this
        doesn't delete the actual collected torrents on disk.
        @param publisher_id PermID
        """
        sql = "Delete From ChannelCast where publisher_id==?"
        self._db.execute_write(sql,(bin2str(publisher_id),))
    
    def updateMyChannelName(self, name): 
        """
        Change the name of your channel
        @param name ASCII-string (TODO: allow Unicode)
        """
        sql = "update ChannelCast set publisher_name==? where publisher_id==?"
        self._db.execute_write(sql,(name,bin2str(self.my_permid),))

    def addTorrents(self,tuples):
        """
        @param tuples (publisher_id, publisher_name, infohash, torrenthash, torrentname, timestamp, signature) in binary and Unicode (names)
        """
        records = []
        for tuple in tuples:
            record = (bin2str(tuple[0]), tuple[1], bin2str(tuple[2]), bin2str(tuple[3]), tuple[4], tuple[5], bin2str(tuple[6]))
    ...
    def selectTorrentsToCollect(self, publisher_id = None):
        # NEW
        """ 
        Returns infohashes of torrents in channel "publisher_id" not yet
        in local database.
        @param publisher_id PermID
        @return [infohash]
        """
        if publisher_id:
            ...
            records = self._db.fetchall(sql,(bin2str(publisher_id),))
        else:
        ...
        infohashes = []
        for record in records:
            infohash = str2bin(record[0])
            infohashes.append(infohash)
        return infohashes
        
    def getNrTorrentsDownloaded(self, publisher_id):
        """ 
        Returns number of downloaded torrents in channel "publisher_id".
        @param publisher_id PermID
        @return int
        """
        ...
        return self._db.fetchone(sql, (bin2str(publisher_id),))

    def getNrTorrentsInChannel(self, publisher_id):
        """ 
        Returns known number of torrents in channel "publisher_id".
        @param publisher_id PermID
        @return int
        """
    ...
    def getRecentAndRandomTorrents(self,NUM_OWN_RECENT_TORRENTS=15,NUM_OWN_RANDOM_TORRENTS=10,NUM_OTHERS_RECENT_TORRENTS=15,NUM_OTHERS_RANDOM_TORRENTS=10):
        """ 
        Return a mix of recent and random ChannelCast records as a list of tuples:
        @return [(publisher_id,publisher_name,infohash,torrenthash,torrentname,time_stamp,signature)]
        """ 
        ...
        myrecenttorrents = self._db.fetchall(sql,(bin2str(self.my_permid),NUM_OWN_RECENT_TORRENTS,))
        ...
        if myrecenttorrents is not None and len(myrecenttorrents)>=NUM_OWN_RECENT_TORRENTS:
            ...
            myrandomtorrents = self._db.fetchall(sql,(bin2str(self.my_permid),t,NUM_OWN_RANDOM_TORRENTS,))
        ...
        othersrecenttorrents = self._db.fetchall(sql,(bin2str(self.my_permid),NUM_OTHERS_RECENT_TORRENTS,))
        ...
        if othersrecenttorrents is not None and len(othersrecenttorrents)>=NUM_OTHERS_RECENT_TORRENTS:
            ...
            othersrandomtorrents = self._db.fetchall(sql,(bin2str(self.my_permid),t,NUM_OTHERS_RANDOM_TORRENTS,))
            allrecords.extend(othersrandomtorrents)
            
        interesting_records = self._getInterestingChannelsTorrents(55-len(allrecords))    
    ...
    def getTorrentFromPublisherId(self, publisher_id, infohash):
        """ Return Torrent database record for item "infohash" in channel 
        "publisher_id".
        @param publisher_id PermID
        @param infohash  20-byte string
        @return dict following Torrent table schema.
        """
        value_name = ['torrent_id',
                      'T.infohash',
                      'name',
        ...
        where = "T.infohash == C.infohash AND publisher_id=='%s' AND t.infohash=='%s' AND name<>''"% (bin2str(publisher_id), bin2str(infohash))
        
        result = self._db.getOne('Torrent T, ChannelCast C', value_name, where)
        value_name[1] = 'infohash'
        
        print >>sys.stderr,"ChannelCastDBHandler: getTorrentFromPublisherId:",`publisher_id`,`infohash`,"DB returned",`result`
    ...
    def getTorrentsFromPublisherId(self, publisher_id, keys = None): ##
        """ Returns list of Torrent database records published in channel 
        "publisher_id".
        @param publisher_id PermID
        @return list of dicts following Torrent table schema
        """
        if keys == None:
            keys = ['torrent_id',
                    'infohash',
        ...
        where = "T.infohash == C.infohash AND publisher_id=='%s' AND name<>''"% bin2str(publisher_id)
    ...
    def getInfohashesForChannel(self, publisher_id):
        """ Returns list of the infohashes of the torrents published in channel 
        "publisher_id".
        @param publisher_id PermID
        @return list of infohashes
        """
        sql = "select infohash from ChannelCast where publisher_id==? ;"
        tuples = self._db.fetchall(sql,(bin2str(publisher_id),))
        infohashes = []
        for tuple in tuples:
            infohashes.append(str2bin(tuple[0]))
        return infohashes
    

    def isItemInChannel(self,publisher_id,infohash):
        """ Returns whether the given infohash is published in channel 
        "publisher_id"
        @param publisher_id PermID
        @param infohash 20-byte string
        @return Boolean
        ...
        isAvailable = self._db.fetchone(sql,(bin2str(publisher_id),bin2str(infohash)))
        if isAvailable:
            return True
        return False
    ...
    def getChannel(self, publisher_id):
        """
        Return info and stats about channel "publisher_id"
        @param publisher_id PermID 
        @return (permid, name, max_timestamp, nr_favorites, nr_torrents, nr_spam, my_vote) tuple
        """
        sql = "Select publisher_id FROM ChannelCast WHERE publisher_id == ?"
        channels = self._getChannels(sql, (bin2str(publisher_id),))
        if len(channels) > 0:
            return channels[0]
   
    def getAllChannels(self):
        """ Return stats and info about all the channels 
        @return list of (permid, name, max_timestamp, nr_favorites, nr_torrents, nr_spam, my_vote) tuples.
        """
        sql = "Select distinct publisher_id FROM ChannelCast"
        return self._getChannels(sql)
    
    def getNewChannels(self, updated_since = None):
        """ Returns info and stats about all newest unsubscribed channels, i.e. 
        the ones with no votes (positive or negative)
        @param updated_since timestamp
        @return list of (permid, name, max_timestamp, nr_favorites, nr_torrents, nr_spam, my_vote) tuples.
        """
        if updated_since:
    ...
    def getLatestUpdated(self, max_nr = 20):
        """ Returns info and stats about all max_nr latest updated channels. 
        @param max_nr int
        @return list of (permid, name, max_timestamp, nr_favorites, nr_torrents, nr_spam, my_vote) tuples.
        """
    ...
    def getMostPopularChannels(self, max_nr = 20):
        """ Returns info and stats about all Top-max_nr most popular channels. 
        @param max_nr int
        @return list of (permid, name, max_timestamp, nr_favorites, nr_torrents, nr_spam, my_vote) tuples.
        """
    ...
    def getMySubscribedChannels(self):
        """ Returns info and stats about the channel I subscribe to. 
        @return list of (permid, name, max_timestamp, nr_favorites, nr_torrents, nr_spam, my_vote) tuples.
        """
        #Sometimes we have no actual channelcast entries, but do know this channel
        sql = "Select mod_id FROM VoteCast Where voter_id = ? AND vote == 2"
        my_sub_channels = [str2bin(mod_id) for mod_id, in self._db.fetchall(sql, (bin2str(self.my_permid),))]        
        ...
        for channel in my_sub_channels:
            channels_with_content.append((channel, '', -1, -1, -1, -1, -1))
        return channels_with_content
    
    def getTimeframeForChannel(self, publisher_id):
        """ Returns oldest and newest timestamp and number of infohashes
        channel "publisher_id".
        @param publisher_id PermId 
        @return (min_timestamp, max_timestamp, nr_torrents) tuple.
        """
    ...
    def getSubscribersCount(self,publisher_id):
        """ 
        Returns the number of subscribers to the channel "publisher_id"
        @param publisher_id PermID
        @return int
        """
        sql = "select count(*) from VoteCast where mod_id==? and vote=2"
        numrecords = self._db.fetchone(sql, (bin2str(publisher_id),))
        return numrecords


    def getMostPopularChannelFromTorrent(self, infohash):
        """ 
        Returns tuple of most popular channel that contains infohash
        @param infohash 20-byte string
        @return (permid,channel_name,#votes,MYSTDICT)
        """
        ...
        if len(publishers) == 0:
            ...
            for publisher_id, in publishers:
                ...
                neg = vcdb.getNumNegVotes(publisher_id)
                pos = vcdb.getNumSubscriptions(publisher_id)
                num_subscribers = pos - neg 
            ...
            return (str2bin(max_id), publisher_name, maxvote, {})
    
    def _getChannels(self, sql, args = None, maxvotes = sys.maxint, minvotes = 0, cmpF = None):
        """Returns the channels based on the input sql, if the number of 
        positive votes is less than maxvotes and the number of torrent > 0.
        @param 
        @return list of (permid, name, max_timestamp, nr_favorites, nr_torrents, nr_spam, my_vote) tuples.
        """
        channels = []
        ...
        for result in results:
            nr_favorites = self.votecast_db.getNumSubscriptions(result[0])
            nr_spam = self.votecast_db.getNumNegVotes(result[0])
            ...
            if nr_votes >= minvotes and nr_votes <= maxvotes:
                ...
                # Arno, 2011-03-24: Don't do GUI formatting in DB layer!
                # name = name[:40] #max 40 characters long
                my_vote = self.votecast_db.getVote(result[0], bin2str(self.my_permid))
                
                channels.append((str2bin(result[0]), name, max_timestamp, nr_favorites, nr_torrents, nr_spam, my_vote))
    ...
    def _getInterestingChannelsTorrents(self, NUM_INTERESTING_TORRENTS=10):
        """ Gets torrents belonging to channels which the peer has not 
        subscribed to, but have been downloaded from. This is done in the hope 
        that channels spread faster even when not subscribed but downloaded,
...
class RichMetadataDBHandler:
    """ 
    Arno: simple DB Handler for UNIKLU's RichMetadata table.
    Not a singleton as it has no state
    """

    def __init__(self):
        self._db = None

    def search(self,query,local=True, maxhits=0):
        t1 = time()
        value_name = ['torrent_id',
                      'infohash',
                      'name',
        ...
        # names of the richmetadata database fields
        richmeta_value_name = ['torrent_id',
                               'aspect_ratio',
                               'audio_coding',
                               'bit_rate',
                               'caption_language',
                               ...
                               'minimum_age',
                               'num_of_channels',
                               'originator',          # mpeg-7 producer
                               'production_date',
                               'production_location',
                               'publisher',           # mpeg-7 disseminator
                               'related_content',
                               'release_date',
                               'release_information',
                               'sign_language',
        ...
        self._db = SQLiteCacheDB.getInstance()
        sql,args = generateSQLQuery(query, richmeta_value_name, maxhits)
        richmeta_results = self._db.fetchall(sql,args)
    
        print >>sys.stderr,"RichDBHandler: searc: got Rich table results",len(richmeta_results)
        
        # make a comma seperated string including all torrent_ids
        tidlst=list()
        for row in richmeta_results:
            tidlst.append(str(row[0]))
        tids = ",".join(tidlst)
  
        mainsql = """select T.*, C.publisher_id as channel_permid, C.publisher_name as channel_name 
                     from Torrent T LEFT OUTER JOIN ChannelCast C on T.infohash = C.infohash 
                     where T.torrent_id in (%s) and T.status_id == 1 order by T.num_seeders desc """ % (tids)
    
        
        if DEBUG:
            print >>sys.stderr, mainsql           
        results = self._db.fetchall(mainsql)
        
        
        print >>sys.stderr,"RichDBHandler: searc: got Torrent table results",len(results)
        ...
        for result in results:
            ...
            rm4torrent = None
            # map correct richmetadata tuple to torrent
            for rm in richmeta_results:
                if rm[0] == torrent['torrent_id']:
                    rm4torrent = dict(zip(richmeta_value_name,rm))
                    break
               
            # perform sanity checks and store richmetadata in result record 
            if rm4torrent is not None:   
                if rm4torrent['aspect_ratio']:
                    # aspect ratio may be given like: '4:3' '1.77:1'
                    m = re.search('\d*\.?\d+:\d*\.?\d+', rm4torrent['aspect_ratio'])
                    if m:
                        torrent['aspectratio'] = rm4torrent['aspect_ratio']
                    else: # aspect ratio might be stored as a number
                        try:
                            ratio = float(rm4torrent['aspect_ratio'])
                            if ratio > 0:
                                torrent['aspectratio'] = rm4torrent['aspect_ratio']
                        except ValueError:
                            pass
                    
                if rm4torrent['audio_coding']:
                    torrent['audiocoding'] = rm4torrent['audio_coding']
                    
                if rm4torrent['bit_rate'] is not None and rm4torrent['bit_rate'] > 0:
                    torrent['bitrate'] = rm4torrent['bit_rate']
                    
                if rm4torrent['caption_language'] is not None and len(rm4torrent['caption_language']) == 2:
                    torrent['captionlanguage'] = rm4torrent['caption_language']
                    
                if rm4torrent['content_type']:
                    torrent['contenttype'] = rm4torrent['content_type']
                          
                if rm4torrent['copyright_notice']:
                    torrent['copyrightstr'] = rm4torrent['copyright_notice']
                    
                if rm4torrent['duration'] is not None and rm4torrent['duration'] > 0:
                    torrent['duration'] = rm4torrent['duration']
                elif rm4torrent['file_size'] > 0 and rm4torrent['bit_rate'] > 0:
                    torrent['duration'] = rm4torrent['file_size']/rm4torrent['bit_rate'];
                    
                if rm4torrent['file_format'] is not None and len(rm4torrent['file_format']) > 0:
                    torrent['fileformat'] = rm4torrent['file_format']
                    
                if rm4torrent['file_size'] is not None and rm4torrent['file_size'] > 0:
                    torrent['filesize'] = rm4torrent['file_size']
                    
                if rm4torrent['frame_rate'] is not None and rm4torrent['frame_rate'] > 0:
                    torrent['framerate'] = rm4torrent['frame_rate']
                    
                if rm4torrent['genre']:
                    torrent['genre'] = rm4torrent['genre']
                    
                if rm4torrent['horizontal_size'] is not None and rm4torrent['horizontal_size'] > 0:
                    torrent['width'] = rm4torrent['horizontal_size']
                    
                if rm4torrent['language'] is not None and len(rm4torrent['language']) == 2:
                    torrent['language'] = rm4torrent['language']
                    
                if rm4torrent['minimum_age'] is not None and rm4torrent['minimum_age'] >= 0:
                    torrent['minimumage'] = rm4torrent['minimum_age']    
                    
                if rm4torrent['num_of_channels'] is not None and rm4torrent['num_of_channels'] > 0:
                    torrent['audiochannels'] = rm4torrent['num_of_channels']
                            
                if rm4torrent['originator']:
                    torrent['producer'] = rm4torrent['originator']
                     
                # try to parse date string to see if it has correct format
                if rm4torrent['production_date']:
                    try:
                        datetime.strptime(rm4torrent['production_date'],'%Y-%m-%d')
                        torrent['productiondate'] = rm4torrent['production_date']
                    except ValueError:
                        pass
                    
                if rm4torrent['production_location'] is not None and len(rm4torrent['production_location']) == 2:
                    torrent['productionlocation'] = rm4torrent['production_location']
                    
                if rm4torrent['publisher']:
                    torrent['disseminator'] = rm4torrent['publisher']
                    
                # ignored related_content (currently not part of the MPEG-7 desc)
                
                # try to parse release date string to see if it has correct format
                if rm4torrent['release_date']:
                    try:
                        datetime.strptime(rm4torrent['release_date'],'%Y-%m-%d')
                        torrent['releasedate'] = rm4torrent['release_date']
                    except ValueError:
                        pass
                   
                # ignored release_information (currently not part of MPEG-7 desc)
                
                if rm4torrent['sign_language'] is not None and len(rm4torrent['sign_language']) == 2:
                    torrent['signlanguage'] = rm4torrent['sign_language']
                    
                if rm4torrent['synopsis']:
                    torrent['comment'] = rm4torrent['synopsis']
                    
                if rm4torrent['title_episode_title']:
                    torrent['episode'] = rm4torrent['title_episode_title']
                # TODO: check URL more detailed
                if rm4torrent['title_image'] is not None and rm4torrent['title_image'][0:4] == "http":
                    torrent['titleimgurl'] = rm4torrent['title_image']
                    
                if rm4torrent['title_main']:
                    torrent['name'] = rm4torrent['title_main']
                    
                if rm4torrent['title_series_title']:
                    torrent['series'] = rm4torrent['title_series_title']
                        
                if rm4torrent['vertical_size'] is not None and rm4torrent['vertical_size'] > 0:
                    torrent['height'] = rm4torrent['vertical_size'];
                    
                if rm4torrent['video_coding']:
                    torrent['videocoding'] = rm4torrent['video_coding']        
            ...
            print >>sys.stderr,"RichDBHandler: search: Adding",`torrent['name']` 
            
            torrents_dict[torrent['infohash']] = torrent
            # the torrent db stores category, status and source id information
            torrent_db = TorrentDBHandler.getInstance()
            try:
                torrent['source'] = torrent_db.id2src[torrent['source_id']]
            except:
                print_exc()
                # Arno: RSS subscription and id2src issue
                torrent['source'] = 'http://some/RSS/feed'
            
            torrent['category'] = [torrent_db.id2category[torrent['category_id']]]
            torrent['status'] = torrent_db.id2status[torrent['status_id']]
            torrent['simRank'] = ranksfind(None,torrent['infohash'])
            torrent['infohash'] = str2bin(torrent['infohash'])
            #torrent['num_swarm'] = torrent['num_seeders'] + torrent['num_leechers']
            torrent['last_check_time'] = 0 #torrent['last_check']
    ...
    def deleteTorrentByID(self, torrent_id, commit=True):
        """
        Updates the RichMetadata table to reflect the change that a Torrent
        has been deleted.
        ...
        """
        self._db = SQLiteCacheDB.getInstance()
        self._db.delete('RichMetadata', commit=commit, torrent_id=torrent_id)
...
# Arno, 2011-02-25: UserLogDBHandler moved to SqliteUserEventLogCacheDB.py
...
# takes a duration in ISO8601 format (e.g.P0Y0M0DT2H1M3S) 
# and converts it to seconds            
def iso8601_duration2seconds(duration):
    
    # regular expression to parse ISO duration strings. 
    # source: isodate package: http://pypi.python.org/pypi/isodate
    ...
    try:
        ...
        # get values without {D,H,M,S} suffix
        days = groups.get('days', '0D')[:-1]
        hours = groups.get('hours', '0H')[:-1]
        mins = groups.get('minutes', '0M')[:-1]
        secs = groups.get('seconds', '0S')[:-1]
