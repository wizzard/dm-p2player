...
argsdef = [('name', '', 'name of the stream'),
           ...
           ('nuploads', 100, 'the max number of peers to serve directly'),
           ...
           ('peerassist', False, 'add source and aux seeders as predef seeds to .tstream. Handle one to each peer on tracker req.')]
           ]
...
if __name__ == "__main__":
    ...
    # STBSPEED 300B
    #auxseeders = [('130.37.193.64',7762)]
    auxseeders = []
    if config['peerassist']:
        predefseeds = []
        sourceipport = (s.get_external_ip(),s.get_listen_port())
        predefseeds.append(sourceipport)
        for auxipport in auxseeders:
            predefseeds.append(auxipport)
    ...
    # STBSPEED M405 try batch upload
    # Arno, 2011-12-09: Disabled for server side in production -stb-speedup-server branch
    #dscfg.set_upload_unit_size(16384)

    # STBSPEED M405: eliminate any extra HAVEs
    dscfg.set_breakup_seed_bitfield(False)

    # STBSPEED 300B
    if len(auxseeders) > 0:
        dscfg.set_live_aux_seeders(auxseeders)
