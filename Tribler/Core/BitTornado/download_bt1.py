...
from CurrentRateMeasure import Measure,TimeKeeper
...
class BT1Download:    
    def __init__(self, statusfunc, finfunc, errorfunc, excfunc, logerrorfunc, doneflag, 
        ...
        # Arno, 2011-02-14: Make G2G configurable, such that it can be enabled
        # on VOD seeders.
        if 'use_g2g' in config:
            if config['use_g2g'] == G2G_AUTO:
                # Automatic i.e. if VOD then G2G, if live then BT 
                self.use_g2g = self.play_video and not ('live' in response['info'])
            elif config['use_g2g'] == G2G_ON:
                self.use_g2g = True
            else:
                self.use_g2g = False
        else:
            # Old behaviour is auto:
            self.use_g2g = self.play_video and not ('live' in response['info'])
...
# _2fastbt
        ...
        self.choker = Choker(config, rawserver.add_task, 
                             self.picker, self.choker_am_I_finished)
    ...
    def set_videoinfo(self,videoinfo,videostatus):
        ...
        if self.play_video or self.svc_video:
            ...
            # Arno, 2011-09-12: let choker know about videostatus
            self.choker.set_videostatus( self.videostatus )
    ...
    def _make_upload(self, connection, ratelimiter, totalup):
        return Upload(connection, ratelimiter, totalup, 
                      ...
                      self.config,last_upload_time=self.last_upload_time)
...
#            self.upmeasure.update_rate(x)
    ...
    def startEngine(self, ratelimiter = None, vodeventfunc = None):
        ...
        self.last_upload_time = TimeKeeper(0.0)

        if ratelimiter:
            self.ratelimiter = ratelimiter
        else:
            self.ratelimiter = RateLimiter(self.rawserver._add_task, 
                                           self.config['upload_unit_size'], 
                                           self.setConns,
                                           self.config['ratelim_min_resched_time'])
...
# 2fastbt_
        ...
        self.connecter = Connecter(self.response, self._make_upload, self.downloader, self.choker, 
                            ...
                            self.rawserver.add_task, self.coordinator, self.helper, self.get_extip_func, self.port, self.use_g2g,self.infohash,self.response.get('announce',None),self.info.has_key('live'),self.response.get('predef seeders',[]))
# _2fastbt
        ...
        if "initial peers" in self.response:
            ...
            dnsidlist = []
            for ipport in self.response["initial peers"]:
                tup = (ipport[0],ipport[1])
                dnsid = (tup,0)
                dnsidlist.append(dnsid)
            self.encoder.start_connections(dnsidlist)
#--- 2fastbt_
        if DEBUG:
        ...
        try:
            def s(self = self, nconns = nconns):
                ...
                if self.connecter is not None:
                    self.connecter.limit_connections()
            if networkcalling:
                s()
            else:
                self.rawserver.add_task(s)
...
#            self.rawserver.add_task(r)
    ...
    def handle_vod_stall(self):
        """ Called by MovieTransporter when no progress has been made on VOD """
        try:
            print >>sys.stderr,"BT1Download: handle_vod_stall"
            npeers = len(self.downloader.downloads)
            if npeers == 1:
                # Stalling while having 1 peer.
                print >>sys.stderr,"BT1Download: handle_vod_stall: 1 peer only! Reconnecting..."
                self.connecter.close_all()
    ...
    def choker_am_I_finished(self):
        return self.finflag.isSet() or self.am_video_source
