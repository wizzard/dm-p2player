...
class DownloadState(Serializable):
    ...
    def get_num_peers(self):
        ...
        to see if there is any progress when non-fatal errors have occured
        (e.g. tracker timeout). This includes connections to any HTTP seeders.
        @return An integer.
        """
        if self.stats is None:
            return 0
    ...
    def get_peerlist(self):
        ...
        'utotal' = Total uploaded from peer in bytes
        'dtotal' = Total downloaded from peer in bytes 
        'completed' = Fraction of download completed by peer (0-1.0) 
        'speed' = The peer's current total download speed (estimated)
        'g2g' = whether the peer uses Give-to-Get
        'g2g_score' = The peer's Give-to-Get score (list of two floats) 
        'pex_received' = RePEX: number of PEX messages received 
        'extended_version' = Client name and version in UTF-8 as sent in extension protocol 'v' handshake field. 
        </pre>
        g2g is only available for non-HTTP seeds, pex_received only if RePEX
        is enabled. Note: update,downrate,utotal and dtotal are in bytes, 
