...
class TorrentDef(ContentDefinition,Serializable,Copyable):
    ...
    def __init__(self,input=None,metainfo=None,infohash=None):
        ...
        self.input['files'] = []
        self.input['creation date'] = long(time.time())
    ...
    def set_initial_peers(self,value):
        """ Set the initial peers to connect to. Clients will automatically
        connect to these peers on start of download. 
    ...
    # PREDEFSEEDS
    def set_predef_seeders(self,value):
        """ Set the predefined seeders. This is used for intra-server 
        communication only (i.e., a tracker directive).  
        @param value List of (IP,port) tuples """
        
        cleanvalues = []
        for ipport in value:
            # bencode can't handle tuples
            if isinstance(ipport, tuple):
                listipport = [ipport[0],ipport[1]]
                cleanvalues.append(listipport)
            else:
                cleanvalues.append(ipport)
        value = cleanvalues
        
        self.input['predef seeders'] = value

    def get_predef_seeders(self):
        """ Returns the list of predefined seeders.
        @return List of (IP,port) tuples. """
        if 'predef seeders' in self.input:
            return self.input['predef seeders']
        else:
            return []


    # Arno, 2011-07-13: Allow user-defined at PDD/ULANCs request
    def set_creation_date(self,value):
        """ Set the creation date of the definition as
        a number of seconds after 1-1-1970 UTC 
        @param value Long
        """
        if self.readonly:
            raise OperationNotPossibleAtRuntimeException()

        self.input['creation date'] = value
        self.metainfo_valid = False


    def get_creation_date(self):
        """ Return creation date as seconds after 1-1-1970 UTC 
        @return long
        """
        return self.input['creation date']
    ...
    def save(self,filename):
        ...
        #
        # Arno, 2011-09-12: I need the ability to save for peer-assisted
        # Downloads where the seeders are always known to the client
        #
        if 'initial peers' in self.metainfo:
            print >>sys.stderr,"WARNING: saving initial peers, not suited for MagnetLink retrieved torrents!"
