...
def RFC3339format2time(timestr):
    formatstr = "%Y-%m-%dT%H:%M:%S"

    # Python datetime has problems with %z, so do RFC3339 ourselves:
    if timestr[-1:] == "Z":
        # UTC
        dt = datetime.datetime.strptime(timestr[:-1], formatstr)
        
        return time.mktime(dt.timetuple())
    else:
        # numoffset: +HH:MM or -HH:MM
        no = timestr[-6:]
        hstr = no[2:3]
        mstr = no[4:6]
        nodifft = int(hstr)*3600 + int(mstr)*60
        
        if no[0] == "-":
            nodifft *= -1
        dt = datetime.datetime.strptime(timestr[:-6], formatstr)
