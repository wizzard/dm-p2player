...
class WebIFPathMapper(AbstractPathMapper):
    ...
    def doget(self,urlpath):
        ...
        if len(path) == 0:    
            ...
            # TIMOPORTAL
            if path.startswith("set_global_speed_upload"):
                try:
                    speedstr = path[path.find('@')+1:]
                    speed = float(speedstr)
                    self.bgApp.set_playerconfig('total_max_upload_rate',speed)
                    self.bgApp.save_playerconfig()
                    
                    txt = "Global upload speed set succesfully to " + speedstr
                    dataStream = StringIO(txt)
                    return {'statuscode':200, 'mimetype':'text/html', 'stream':dataStream, 'length': len(txt)}
                except Except,e:
                    return {'statuscode': 500, 'statusmsg':'Global upload speed set failed ' + str(e) }
            ...
            # retrieve resource such as pages or images
            if urlpath[6] == '/' and os.path.isfile(absPath):
                ...
                # Arno, 2011-02-15: WARNING: this code is jQuery 1.4.x dependent,
                # i.e. it assumes a extra & after the /webUI?, e.g.
                #
                #   http://127.0.0.1:6876/webUI?&{%22method%22:%22get_speed_info%22}
                #
                # jQuery 1.5 doesn't have that. 
...
#            infohash = (d.get_def().get_infohash()).toJSON()
            ...
            downloaded = self.humanize_bytes(length * progress)

            dl = {'id' : infohash, 'name': tdef.get_name(), 'status': dlstatus_strings[ds.get_status()], 'progress': ds.get_progress(), 'upload': ds.get_current_speed(UPLOAD), 'download': ds.get_current_speed(DOWNLOAD), 'size': self.humanize_bytes(length), 'downloaded': downloaded }
