...
def usage():
    ...
    print "\t--buddycast\t\t\tannounce tstreams on buddycast"
    print "\t--installdir\t\t\tinstall dir"
    print "\t--peerassist\t\t\tenable peer-assisted mode"
    print "\t--version"
...
def main():
    ...
    for o, a in opts:
        if o in ("-h", "--help"):
            ...
            tracking = False
        elif o in ("--peerassist"):
            peerassist = True
        elif o in ("--buddycast"):
            buddycast = True
        elif o in ("--installdir"):
            installdir = a
        elif o in ("-v", "--version"):
            print_version()
            sys.exit(0)
    ...
    if installdir is not None:
        sscfg.set_install_dir(installdir)
    sscfg.set_listen_port(port)

    sscfg.set_social_networking(False)
    sscfg.set_bartercast(False)
    sscfg.set_torrent_collecting(False) # only announce own
    if not buddycast:
        sscfg.set_megacache(False)
        sscfg.set_buddycast(False)
    else:
        print >>sys.stderr,"Enabling buddycast for content gossip"
    ...
    if tracking:
        ...
        # M405 STBSPEEDCONFIG
        sscfg.set_tracker_reannounce_interval(60)
        
    # PREDEFSEEDS
    #
    # Enable to let the internal tracker send out one of the
    # seeders predefined in the .tstream file as first hit in a tracker reply.
    # If all seeders are online, a peer will always receive the same seeder addr.
    # In other words, this is for peer-assisted scenarios.
    #
    if peerassist:
        print >>sys.stderr,"Enabling peer assistance. Make sure .tstreams are created with predef seeders set!"
        sscfg.set_tracker_send_predefseeds(True)
    ...
    # Arno, 2011-02-14: Enable G2G on seeder.
    dscfg.set_give_to_get(G2G_ON)
    
    # STBSPEED 300B: Previously got 200 mbps out of a single process. This
    # means ~100 slots at 2 mbps. 
    dscfg.set_max_uploads(100)
