...
class PlayerOptionsDialog(wx.Dialog):
    ...
    def OnApply(self,event = None):
        ...
        if self.port is not None and self.port != self.wxapp.s.get_listen_port():
            dlg = wx.MessageDialog(None, self.wxapp.appname+" will now exit to change the port. Reload the Web page to restart it", self.wxapp.appname+" Restart", wx.OK|wx.ICON_INFORMATION)
...
class PlayerAdvancedOptionsDialog(wx.Dialog):
    ...
    def __init__(self,icons,port,wxapp):
        ...
        wx.Dialog.__init__(self, None, -1, self.wxapp.appname+' Advanced Options', size=(400,200), style=style)
