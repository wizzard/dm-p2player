...
#
""" Tribler Core API v1.2.0, Dec 2011. Import this to use the API """

# History:
#
# 1.2.0      Released with Next-Share M48
#
# 1.2.0rc1   Support for swift Downloads in Core API:
#            Refactored TorrentDef into generic ContentDefinition and 
#            TorrentDef+SwiftDef as subclass implementations.
#            Added: [s/g]et_swift_*
#
#            To optimize sharing on the NextShareTV set-top box we made a 
#            number of Core parameters configurable which previously were not: 
#            (M405 STBSPEEDCONFIG):
#
#            - SessionConfig.[s/g]et_socket_write_always()
#            - SessionConfig.[s/g]et_socket_rcvbuf_size()
#            - SessionConfig.[s/g]et_socket_sndbuf_size()
#            - DownloadConfig.[s/g]et_live_drop_pieces()
#            - DownloadConfig.[s/g]et_upload_while_prebuf()
#            - DownloadConfig.[s/g]et_ratelim_min_resched_time()
#
#            Added a peer-assisted mode of operation where the NS tracker
#            sends the address of a well-known seeder in every reply. It 
#            automatically distributes the load over the available seeders.
#            To this extent, we added the following methods to the API:
#            - TorrentDef.[s/g]et_predef_seeders()
#            - SessionConfig.[s/g]et_tracker_send_predefseeds()
#
# 1.1.1      Released with Next-Share M40.5
#
# 1.1.1rc1   TorrentDef creation date user-defined at PDD/ULANCs request.
#
