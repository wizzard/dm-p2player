...
##Changed from 6 to 7 for Raynor's TermFrequency table + Niels' UserEventLog
##Changed from 7 to 8 for moving Niels' UserEventLog to separate DB and to 
##                    enable UNIKLU rich metadata search support
...
class SQLiteCacheDBV8(SQLiteCacheDBBase):
    def updateDB(self, fromver, tover):

        if fromver == 6:
            # Arno, 2011-03-24: DIRTY HACK
            # Database schemas evolved independently between NextShare
            # and Tribler. So in:
            #    NextShare (NSPC 1.3.0) v6 = v5 + UNIKLU Richmetadata table
            #    Tribler 5.3.x          v7 = v5 + v6 (never published) + v7
            #
            # Workaround is to set fromver to 5 for NextShare, and make
            # CREATE Richmetadata conditional
            fromver = 5
...
-- v6 RichMetadata DB
CREATE TABLE IF NOT EXISTS RichMetadata (
    richmetadata_id     integer PRIMARY KEY ASC AUTOINCREMENT NOT NULL,
    ...
    release_date        text,
    release_information text,
    sign_language       text,
    synopsis            text,
    title_episode_title text,
    title_image         text,
    title_main          text,
    title_series_title  text,
...
"""
        ...
        # Arno, 2011-03-24: Need ChannelCast table first
        # Channelcast
        if fromver < 6:
            ...
            self.execute_write(sql, commit=True)
        ...
        if fromver < 7:
            ...
            if TEST_SQLITECACHEDB_UPGRADE:
                ...
                extractor = TermExtraction.getInstance(installdir=installdir)
            else:
...
class SQLiteCacheDB(SQLiteCacheDBV8):
