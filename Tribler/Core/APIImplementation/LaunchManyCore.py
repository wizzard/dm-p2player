...
from Tribler.Core.ClosedSwarm.ECS_ClosedSwarms import ECS_Manager
...
class TriblerLaunchMany(Thread):
    ...
    def register(self,session,sesslock):
        ...
        self.rawserver = RawServer(self.sessdoneflag,
                                   ...
                                   config['timeout'],
                                   config['socket_write_always'],
                                   config['socket_rcvbuf_size'],
                                   config['socket_sndbuf_size'],
        ...
        if config['megacache']:
            ...
            # Arno, 2011-01-27: NextShare XML Rich Metadata
            ...
            # Arno, 2011-01-27: NextShare XML Rich Metadata
            self.nsrichmetadata_db = RichMetadataDBHandler()
            ...
            if config['crawler']:
                ...
                # Arno, 2011-03-24: Niels' user events
                cachedb.init_usereventlog(config, self.rawserver_fatalerrorfunc)
        ...
        # ECS
        ecsmanager = ECS_Manager.getInstance(keypair_filename=config['eckeypairfilename'])
    ...
    def add(self,tdef,dscfg,pstate=None,initialdlstatus=None):
        ...
        try:
            ...
            # ECS
            torrent_keys = tdef.get_cs_keys_as_ders()
            if len(torrent_keys):
                ecsmanager = ECS_Manager.getInstance()
                ecsmanager.register_torrent(infohash, torrent_keys)
    ...
    def queue_for_hashcheck(self,sd):
        ...
        # Check SEEDING and then smallest torrents first
    ...
    def hashcheck_done(self,success=True):
        ...
        Called by network thread """
        if DEBUG:
            print >>sys.stderr,"tlm: hashcheck_done, success",success,self.sdownloadtohashcheck.b64_infohash
        if success:
    ...
    def update_torrent_checking_period(self):
        ...
        #print >> sys.stderr, "tlm: update_torrent_checking_period: set to", self.torrent_checking_period
    ...
    def setup_multicast_discovery(self):
        ...
        if LIBRARYNAME == "BaseLib":
            # Arno, 2011-01-20: to prevent overlays linking up e.g. due to multicast
            # discover, use different group
            ipv4 = '224.0.1.44'
            ipv6 = 'ff02::4124:1261:ffdf' 
            port = '32108' 
        else:
            ipv4 = '224.0.1.43'
            ipv6 = 'ff02::4124:1261:ffef' 
            port = '32109' 
        
        mc_config = {'permid':self.session.get_permid(),
                     'multicast_ipv4_address':ipv4,
                     'multicast_ipv6_address':ipv6,
                     'multicast_port':port,
    ...
    def swift_remove(self,d,removecontent=False):
        """ Called by any thread """
        self.sesslock.acquire()
        try:
...
def singledownload_size_cmp(x,y):
    """ Method that compares 2 SingleDownload objects based on their status
    (SEEDING Downloads first) and then size of the content of the BT1Download 
    ...
    if x is None and y is None:
        ...
        # Arno, 2010-12-13: Sort SEEDING Downloads to start of queue
        if x.get_checkpointedstatus() == DLSTATUS_SEEDING and y.get_checkpointedstatus() != DLSTATUS_SEEDING:
            return -1
        elif y.get_checkpointedstatus() == DLSTATUS_SEEDING and x.get_checkpointedstatus() != DLSTATUS_SEEDING:
            return 1
