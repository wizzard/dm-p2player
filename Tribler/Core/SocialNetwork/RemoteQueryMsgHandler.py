...
class RemoteQueryMsgHandler:
    ...
    def register(self,overlay_bridge,launchmany,config,bc_fac,log=''):
        ...
        self.richmetadata_db = launchmany.nsrichmetadata_db
        # debug
    ...
    def set_user_query_handlers(self,queryprefix2handlercallbackdict):
        """ Called by any thread whilst holding session lock """
        copyqp2callbackdict = copy.copy(queryprefix2handlercallbackdict)
        olbridge_set_user_query_handlers_lambda = lambda:self.olbridge_set_user_query_handlers(copyqp2callbackdict)
        
        # Delegate setting to user handlers to OverlayThread, to avoid concurrency
        self.overlay_bridge.add_task(olbridge_set_user_query_handlers_lambda,0)
        
    def olbridge_set_user_query_handlers(self,qp2callbacks):
        self.userqp2callbacks = qp2callbacks
    ...
    def send_query(self,query,usercallback,max_peers_to_query=MAX_PEERS_TO_QUERY):
        ...
        if max_peers_to_query > 0:
            send_query_func = lambda:self.olthread_send_query_callback(query,usercallback,max_peers_to_query)
    ...
    def send_query_to_peers(self,query,peers,usercallback):
        ...
        if len(peers) > 0:
            send_query_func = lambda:self.olthread_send_query_callback_to_peers(query,peers,usercallback)
            self.overlay_bridge.add_task(send_query_func,0)

    def olthread_send_query_callback(self,query,usercallback,max_peers_to_query):
        ...
        if query.startswith("CHANNEL"):
            ...
            # User-defined query prefixes
            wantminoversion = OLPROTO_VER_FOURTEENTH
        ...
        self.olthread_send_query_callback_to_peers(query, peers, usercallback)
    
    def olthread_send_query_callback_to_peers(self, query, peers, usercallback):
        """ Called by overlay thread """
        query_cache = {}
    ...
    def create_and_register_query_id(self,query,usercallback):
        ...
        queryrec = {'query':query,'usercallback':usercallback,'asktime':time()}
    ...
    def send_callback(self,exc,permid):
        #print "******* query/reply was sent to", show_permid_short(permid), exc
        pass
    ...
    def log(self, permid, decoded_message):        
        ...
        #ip = "x.y.z.1"
        s = "%s\t%s\t%s\t%s\n"% (timestamp, show_permid(permid), ip, decoded_message)
    ...
    def process_query(self, permid, d, selversion):
        if self.process_user_query(permid,d,selversion):
            # User will handle the query
            return
        ...
        print >>sys.stderr,"rqmh: Got query",`netwq`
        ...
        if netwq.startswith("SIMPLE"): # remote query
            ...
            dbhits = self.search_torrents(kws, maxhits=MAX_RESULTS,sendtorrents=sendtorrents)
            p = self.create_remote_query_reply(d['id'],dbhits,selversion)
            
        elif netwq.startswith('CHANNEL'): # channel query
            if DEBUG:
                print>>sys.stderr, "rqmh: Incoming CHANNEL query", d['q']
            q = d['q'][len('CHANNEL '):]
            uq = self.clean_netwq(q,channelquery=True)
            dbhits = self.channelcast_db.searchChannels(uq)
            p = self.create_channel_query_reply(d['id'],dbhits,selversion)


        elif netwq.startswith('RICHMETA+METADATA'): # UNIKLU query
            if DEBUG:
                print >>sys.stderr,"rqmh: RICHMETA+METADATA search for torrents matching",d['q']
            #remove user query prefix (+ whitespace)
            q = netwq[len('RICHMETA+METADATA')+1:]
            # Arno: Query is in own language converted to SQL later, so safe
            dbhits = self.richmetadata_db.search(q,local=False, maxhits=MAX_RESULTS)
            p = self.create_richmeta_plus_metadata_reply(d['id'],dbhits,selversion)

        # Arno, 2011-01-27: Don't send reply if no hits
        if p is not None:
            m = QUERY_REPLY+p
    ...
    def create_remote_query_reply(self,id,dbhits,selversion):
        getsize = os.path.getsize
        join = os.path.join
        d = {}
        d['id'] = id
        d2 = {}
        for torrent in dbhits:
            r = {}
    ...
    def create_channel_query_reply(self,id,dbhits,selversion):
        d = {}
        d['id'] = id
        
        if self.bc_fac.channelcast_core is not None:
            d2 = self.bc_fac.channelcast_core.buildChannelcastMessageFromHits(dbhits,selversion,fromQuery=True)
            d['a'] = d2
        else:
            d['a'] = {}
        return bencode(d)
    ...
    # UNIKLU: for P2P Rich Metadata queries
    def create_richmeta_plus_metadata_reply(self,id,dbhits,selversion):
         
        colltorrdir = self.session.get_torrent_collecting_dir()
        if DEBUG:
            print >>sys.stderr,"rqmh: search_torrents: adding torrents to",len(dbhits),"hits"
        for dbhit in dbhits:
            filename = os.path.join(colltorrdir,dbhit['torrent_file_name'])
            try:
                ...
                metadata = None
                metatype = None
            dbhit['metatype'] = metatype
            dbhit['metadata'] = metadata
            
        # Filter out hits for which we could not read torrent file (rare)
        newdbhits = []
        for dbhit in dbhits:
            if dbhit['metadata'] is not None:
                newdbhits.append(dbhit)
        
        return self.create_remote_query_reply(id, newdbhits, selversion)
        

    #
    # Handler user-defined query prefixes
    #
    def process_user_query(self,permid,d,selversion):
        ueq = d['q'].decode("UTF-8")
        idx = ueq.find(' ')
        if idx == -1:
            print >>sys.stderr,"rqmh: Malformed user-defined query: no space",`d['q']`
            return
        
        prefix = ueq[0:idx]
        handlercallback = self.userqp2callbacks.get(prefix,None)
        if handlercallback is None:
            # Fallover to default query handlers
            return False
        
        # Call into user-space to get answers
        handlercallback(permid,ueq,d['id'],self.user_query_hitscallback)
        
        return True
        
        
    def user_query_hitscallback(self,permid,qid,exc,hits):
        """ User returning hits to query, forward to requesting PermID.
        Called by any thread """
        if exc is None:
            # TODO: optimize: if OverlayThread calling, don't schedule
            olbridge_user_query_hitscallback_lambda = lambda:self.olbridge_user_query_hitscallback(permid,qid,hits)
            self.overlay_bridge.add_task(olbridge_user_query_hitscallback_lambda,0)


    def olbridge_user_query_hitscallback(self,permid,qid,hits):
        """ Called by overlay thread """
        
        # Put user-defined hits into QUERY_REPLY message
        d = {}
        d['id'] = qid
        d['a'] = hits
        payload = bencode(d)
        
        m = QUERY_REPLY+payload

        self.inc_peer_nqueries(permid)

        # Connection may have closed in mean time, reopen if needed:
        reply_conn_callback_lambda = lambda exc,dns,permid,selversion:self.conn_callback(exc,dns,permid,selversion,m)
        self.overlay_bridge.connect(permid,reply_conn_callback_lambda)
    ...
    def recv_query_reply(self,permid,message,selversion):
        ...
        # Test content of hits, is query dependent now
        if not isValidQueryReplyGivenQ(d,queryrec['query'],selversion):
            return False
    ...
    def process_query_reply(self,permid,query,usercallback,d):
        ...
        if DEBUG:
            print >>sys.stderr,"rquery: process_query_reply:",show_permid_short(permid),`query`
    ...
    def purge_old_queryrecs(self):
        dellist = []
        for id,queryrec in self.query_ids2rec.iteritems():
            if queryrec['asktime']+120.0 > time():
                dellist.append(id)
                
        for id in dellist:
            del self.query_ids2rec[id]   
...
def isValidQuery(d,selversion):
    ...
    if idx == -1:
        ...
        return False
    if d['q'].startswith('SIMPLE') or d['q'].startswith('CHANNEL'):
        # Default queries
        try:
            keyws = d['q'][idx+1:]
...
def isValidQueryReply(d,selversion):
    ...
    if not isinstance(d['id'],str):
        if DEBUG:
            print >>sys.stderr,"rqmh: reply: id key not str"
        return False
    if len(d) > 2: # no other keys
        if DEBUG:
...
def isValidQueryReplyGivenQ(d,q,selversion):
    if q.startswith('SIMPLE') or q.startswith('CHANNEL'):
        # Default queries
        if not isinstance(d['a'],dict):
            if DEBUG:
                print >>sys.stderr,"rqmh: reply: a key not dict"
            return False
        if not isValidHits(d['a'],selversion):
            return False
        
        if selversion >= OLPROTO_VER_TWELFTH:
            if q.startswith('SIMPLE+METADATA'):
