...
#        #assert validChannelCastMsg(d)
    ...
    def updateChannel(self,query_permid, query, hits):
        ...
        """
        if DEBUG:
            print >> sys.stderr, "channelcast: sending message to", show_permid_short(query_permid), query, len(hits)
    ...
    def _updateChannelInternal(self, query_permid, query, hits):
        ...
        all_spam_channels = self.votecastdb.getModIdsWithNegVote(self.session.get_permid())
        for k,v in hits.items():
            #check if the record belongs to a channel who we have "reported spam" (negative vote)
            if v['publisher_id'] in all_spam_channels:
                # if so, ignore the incoming record
                continue
            
            hit = (v['publisher_id'],v['publisher_name'],v['infohash'],v['torrenthash'],v['torrentname'],v['time_stamp'],k)
    ...
    def _updateChannelcastDB(self, query_permid, query, hits, listOfAdditions):
        """
        @param hits list of (publisher_id,publisher_name,infohash,torrenthash,torrentname,timestamp,signature) tuples
        binary fields are in binary, names are Unicode objects.
        """
        if DEBUG:
        ...
        if query and query.startswith('CHANNEL p') and len(publisher_ids) == 1:
            ...
            if len(infohashes) > nr_torrents:
                if len(infohashes) > 50 and len(infohashes) > nr_torrents +1: #peer not behaving according to spec, ignoring
                    ...
                    return
                self.channelcastdb.deleteTorrentsFromPublisherId(publisher_id)
            if DEBUG:
            ...
            my_favorites = self.votecastdb.getMySubscriptions()
        ...
        for publisher_id in publisher_ids:
            for infohash in self.channelcastdb.selectTorrentsToCollect(publisher_id):
                missing_infohashes[infohash] = publisher_id
    ...
    def _sequentialQueryPeers(self, publisher_id, peers):
        ...
        def dorequest():
            if peers:
                ...
                q = "CHANNEL p "+show_permid(publisher_id)
    ...
    def buildChannelcastMessageFromHits(self, hits, selversion, dest_permid=None, fromQuery=False):
        ...
        if selversion >= OLPROTO_VER_FOURTEENTH:
            ...
            if DEBUG:
                print >> sys.stderr, "channelcast: creating subtitle-enriched messages "\
                    "since peer has version: ", selversion
        d = {}
        for hit in hits:
            # ARNOUNICODE: temp fixes until data is sent not Base64-encoded
             
            # 08/04/10 Andrea: I substituted the keys with constants, otherwise a change here
