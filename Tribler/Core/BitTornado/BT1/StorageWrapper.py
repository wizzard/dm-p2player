...
class StorageWrapper:
    ...
    def _initialize(self):
        ...
        if DEBUG:
            print >>sys.stderr,"StorageWrapper: _initialize: enter",self.videoinfo['outpath']
        ...
        if self.initialize_next:
            ...
            if not self.initialize_tasks:
                if DEBUG:
                    print >>sys.stderr,"StorageWrapper: _initialize finished"
                self.initialize_done(success=True)
                self.initialize_done = None
                return
            msg, done, init, next = self.initialize_tasks.pop(0)
            ...
            if init():
                if DEBUG:
                    print >>sys.stderr,"StorageWrapper: _initialize: setup successful"

                self.initialize_status(activity = msg, fractionDone = done)
                self.initialize_next = next
            else:
                if DEBUG:
                    print >>sys.stderr,"StorageWrapper: _initialize: setup failed"
        ...
        # Arno, 2010-11-24: if a failed() occurs in the init() or next() methods
        # the SingleDownload gets killed, and thus the queue to which backfunc
        # queues tasks is also closed, so this code does nothing then.
        #
        self.backfunc(self._initialize)
        ...
        try:
            ...
            for index in xrange(len(self.hashes)):
                ...
                if have[index]:
                    ...
                    # Arno, 2010-09-28: Fix checkpointing.
...
#            print_exc()
    ...
    def failed(self,s):
        ...
        self.report_failure(s) # READ BELOW!
        # Arno, 2010-11-24: This would be done twice, report_failure 
        # calls SingleDownload.fatalerrorfunc which calls shutdown()
        # which calls LaunchMany.hashcheck_done, which schedules another Download.
