# Written by Jan David Mol, Arno Bakker, Boudewijn Schoon, Diego Rabaioli
...
from Tribler.Core.Statistics.Status.Status import get_status_holder, TRIALLOG
...
class MovieOnDemandTransporter(MovieTransport):
    ...
    PREBUF_REHOOKIN_SECS = 10.0

    # maximum delay between pops when streaming before we force a restart (seconds)
    MAX_POP_TIME_LIVE = 10
    MAX_POP_TIME_VOD = 10
    
    # LIVEPEERSBIAS V2
    # Enable/disable bias for downloading from peers for live streaming.
    LIVE_PEERS_BIAS = True
    
    # the trust deadline: until X seconds before a piece is due we trust the
    # peers and not the source to deliver us the piece.
    LIVE_PEERS_BIAS_TRUST_TILL_SECS = PREBUF_SEC_LIVE/2.0

    # Probability at which to download from the source, minus exceptions. See
    # PiecePickerStreaming.
    LIVE_PEERS_BIAS_DLPROB = 0.75
    ...
    LIVE_PEERS_BIAS_UPLOAD_IDLE_SECS = 5.0
    ...
    LIVE_PEERS_BIAS_FORWARDER_DLPROB_BONUS = 0.5  # 0..1


    # VODPEERSBIAS
    # Enable/disable bias for downloading from peers for video-on-demand.
    VOD_PEERS_BIAS = True
    
    # Probability at which to download from the source, minus exceptions. See
    # PiecePickerStreaming.
    VOD_PEERS_BIAS_DLPROB = 0.75
    ...
    def calc_live_startpos(self,prebufsize=2,have=False):
        ...
        if DEBUG_HOOKIN:
            print >>sys.stderr,"\nvod: calc_live_startpos: New round: prebuf",prebufsize,"have",have
        ...
        if have:
            ...
            if DEBUG and DEBUG_HOOKIN:
                if sourcehave is not None:
                    print >>sys.stderr,"vod: calc_live_startpos: DEBUG: testing for multiple clients at source IP (forbidden!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!, but used in CS testing). Source have numtrue",sourcehave.get_numtrue()
                    if sourcehave.get_numtrue() < 100:
                        print >>sys.stderr,"vod: calc_live_startpos: Source sent near empty BITFIELD, CS testing bug? Not tuning in as source"
                        sourcehave = None
            ...
            if sourcehave is None:
                ...
                if DEBUG_HOOKIN:
                    print >>sys.stderr,"vod: calc_live_startpos: Connected to source, hookin on that" 
                    #print >>sys.stderr,"vod: calc_live_startpos: sourcehave",`sourcehave.tostring()`
                threshold = 1
        ...
        if numseeds == 0 and totalhaves == 0:
            ...
            if DEBUG_HOOKIN:
                print >>sys.stderr,"vod: calc_live_startpos: no pieces"
            return False
        ...
        if not vs.wraparound:
            if numseeds > 0 or numhaves[epiece] > 0:
                ...
                if DEBUG_HOOKIN:
                    print >>sys.stderr,"vod: calc_live_startpos: vod mode"
        ...
        for i in xrange(epiece,bpiece-1,-1):
            ...
            #        print >>sys.stderr,"vod: calc_live_startpos: discarding piece %d as it is owned by only %d<%d neighbours" % (i,inspecthave[i],threshold)

            if inspecthave[i] >= threshold:
                maxnum = i
                if not have:
                    if inspecthave == numhaves:
                        print >>sys.stderr,"vod: calc_live_startpos: chosing piece %d as it is owned by %d>=%d neighbours (prewrap)" % (i,inspecthave[i],threshold)
                    else:
                        print >>sys.stderr,"vod: calc_live_startpos: chosing piece %d as it is owned by the source (prewrap)" % (i)
                else:
                    print >>sys.stderr,"vod: calc_live_startpos: chosing piece %d as it is owned by me (prewrap)" % (i)
                        
                break

        if maxnum is None:
            print >>sys.stderr,"vod: calc_live_startpos: Failed to find quorum for any piece"
            return False

        # if there is wraparound, newest piece may actually have wrapped
        if vs.wraparound and maxnum > epiece - vs.wraparound_delta:
            # Arno, 2011-09-27: When ~epiece is the oldest piece we have,
            # the window has wrapped around almost completely and window size
            # is 2 * delta_left, not 1 * delta_left.
            delta_left = (2*vs.wraparound_delta) - (epiece-maxnum)
            ...
            for i in xrange( vs.first_piece+delta_left-1, vs.first_piece-1, -1 ):
                if inspecthave[i] >= threshold:
                    ...
                    if DEBUG_HOOKIN:
                        if not have:
                            if inspecthave == numhaves:
                                print >>sys.stderr,"vod: calc_live_startpos: chosing piece %d as it is owned by %d>=%d neighbours (wrap)" % (i,inspecthave[i],threshold)
                            else:
                                print >>sys.stderr,"vod: calc_live_startpos: chosing piece %d as it is owned by the source (wrap)" % (i)
                        else:
                            print >>sys.stderr,"vod: calc_live_startpos: chosing piece %d as it is owned by me (wrap)" % (i)
                            
                    break

        print >>sys.stderr,"vod: calc_live_startpos: hookin candidate (unfudged)",maxnum
        ...
        if not have and threshold == 1 and oldstartpos is not None:
            ...
            print >>sys.stderr,"vod: calc_live_startpos: m o",maxnum,oldstartpos,"diff",diff,"diffs",diffs
            if diffs < self.PREBUF_REHOOKIN_SECS: 
                print >>sys.stderr,"vod: calc_live_startpos: difference in hook-in point < ",self.PREBUF_REHOOKIN_SECS,"s, keeping old."
                return True
        ...
        try:
            try:
                ...
                if vs.playing and not self.rehookin:
                    ...
                    if vs.live_streaming and self.last_pop is not None and time.time() - self.last_pop > self.MAX_POP_TIME_LIVE:
    ...
    def inc_pos(self):
        ...
        # Arno, 2011-09-27: Off by one error fix.
        if vs.live_streaming:
...
#            return self.playback_pos * self.movieselector.piece_length / self.movieselector.bitrate
    ...
    def refill_buffer( self ):
        ...
        if self.http_support is not None:
            ...
            if DEBUG:
                if self.outbuflen < ( mx / 3 ):
                    print >> sys.stderr, "VideoOnDemand.REFILL_BUFFER len:%d size:%d low:%d high:%d" % (len(self.outbuf),self.outbuflen,mx/4,mx/2)
            ...
            # Boudewijn, 22/09/10: we need to stop this when the
            # download has completed
            if self.storagewrapper.amount_left == 0:
                self.http_support.stop_video_support()

            elif self.outbuflen < ( mx / 4 ):
                self.http_support.start_video_support( 0 )

            elif self.outbuflen > ( mx / 2):
                self.http_support.stop_video_support()
        ...
        if buffer_underrun() and vs.pausable:
            ...
            # STBSPEED M405: vs.dropping currently off, so changed to vs.live_streaming
            if vs.live_streaming: # live
                def sustainable():
        ...
        def push( i, data ):
            ...
            if DEBUG:
                print >>sys.stderr,"vod: trans: %d: pushed l=%d" % (vs.playback_pos,i)
        ...
        for piece in vs.generate_range( vs.download_range() ): 
            ...
            if ihavepiece:
                ...
                if vs.dropping or forcedrop:
                    ...
                    if not vs.live_streaming: # not dropping. STBSPEED M405: only if live   
                        # Arno, 2010-12-14: Current ULANC seeder sometimes stops
                        # sending data. Try VOD "rehook-in", i.e., closing all conns 
                        # and recontact tracker. See PiecePickerStreaming for test.
                        #
                        if self.last_pop is not None and time.time() - self.last_pop > self.MAX_POP_TIME_VOD:
                            self.bt1download.handle_vod_stall()
                            self.last_pop = time.time()
                    break
    ...
    def notify_playable(self):
        ...
        if complete:
            ...
            stream = MovieTransportStreamWrapper(self,self.videoinfo['vod_useroffset'])
    ...
    def live_invalidate_piece_globally(self, piece, mevirgin=False):
        ...
        mevirgin indicates whether we already downloaded stuff,
