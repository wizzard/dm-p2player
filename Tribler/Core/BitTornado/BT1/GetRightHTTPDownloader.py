...
# TODO: 
#       Check that lock never leads to deadlock. Currently SessionPoolThread locks
#       and NetworkThread unlocks.
#
#       def _want is executed by other thread than NetworkThread, concurrency!
#
#       Add/Check that we do timeouts to HTTP requests such that the threads 
#       doing this don't hang indefinitely. Threads are SessionPoolThreads which 
#       should return to the pool at one point.
#
#       And/or resize threadpool automatically.
#
...
DEBUG = False
DEBUG_LOCK = False  # print debug info for locking
...
class haveHighRange:
    """
    The HTTPDownloader will only serve pieces in the high range while
    playing.

    However... when the player is not playing (prebuffer, stall, etc.)
    we will serve anything that the piece picker wants.
    """
    def __init__(self, videostatus):
        self._videostatus = videostatus
    def complere(self):
        return False
    def __getitem__(self, piece_id):
        # check for range
        first, last = self._videostatus.get_high_range()
        if self._videostatus.in_range(min(0, first-1), last+1, piece_id):
            return True

        # check for buffering
        vs = self._videostatus
        return vs.prebuffering or (vs.paused and vs.autoresume)

        # return self._videostatus.in_high_range(piece_id)
...
class SingleDownload(SingleDownloadHelperInterface):
    ...
    def __init__(self, downloader, url):
        ...
        # If Video Support policy is not enabled then use Http seed normaly
    ...
    def _download(self):
        """ Called by SessionPoolThread """
        if DEBUG:
            print "http-sdownload: download()"
        if DEBUG_LOCK:
            print >>sys.stderr,"http-sdownload: lock"
        self.request_lock.acquire()
        if DEBUG:
            print >>sys.stderr,"http-sdownload: got lock"
        if self.is_frozen_by_helper():
            ...
            if DEBUG_LOCK:
                print >>sys.stderr,"http-sdownload: unlock: frozen"
            self.request_lock.release()
            return
        self.cancelled = False
        if self.downloader.picker.am_I_complete():
            # Boudewijn, 24/09/10: Sometimes the download is already
            # removed and this call crashes, causing a deadlock.
            if self in self.downloader.downloads:
                self.downloader.downloads.remove(self)
            
            if DEBUG_LOCK:
                print >>sys.stderr,"http-sdownload: unlock: complete"
            self.request_lock.release()
            return
        ...
        if self.index is None and self.frozen_by_helper:
            ...
            if DEBUG_LOCK:
                print >>sys.stderr,"http-sdownload: unlock: frozen2"
            self.request_lock.release()
            return
        ...
        if self.index is None:
            ...
            if DEBUG_LOCK:
                print >>sys.stderr,"http-sdownload: unlock: nothing picked"
            self.request_lock.release()
        else:
