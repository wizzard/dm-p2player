...
REPLY_HARD_LIMIT = 50  # we will store no more than this number of
...
class DialbackMsgHandler:
    ...
    def register(self,overlay_bridge,launchmany,rawserver,config,buddycastfac):
        """ Called by MainThread """
        self.overlay_bridge = overlay_bridge
        self.rawserver = rawserver
        # Arno, 2011-01-27: Pointer to BuddyCast so we can inform it of connectable peers
        self.buddycastfac = buddycastfac
    ...
    def olthread_process_dialback_request(self,permid,message,selversion):
        ...
        lambda_connect_callback = lambda exc,dns:self.network_returnconn_reply_connect_callback(exc,dns,permid)
        self.returnconnhand.connect_dns(dns,lambda_connect_callback)

        # 4. Message processed OK, don't know about sending of reply though
        return True


    def network_returnconn_reply_connect_callback(self,exc,dns,permid):
        ...
        if exc is None:
            ...
            try:
                ...
                # Arno, 2011-01-27: We now also known he is reachable
                self.network_peer_is_reachable(permid)
            except:
                print_exc()
                return False
        elif DEBUG:
    ...
    def network_peer_is_reachable(self,permid):
        olthread_peer_is_reachable_lambda = lambda:self.olthread_peer_is_reachable(permid)
        self.overlay_bridge.add_task(olthread_peer_is_reachable_lambda,0)

    def olthread_peer_is_reachable(self,permid):
        """ We successfully connected back to a peer, so he is reachable.
        Record in DB and tell BuddyCast
        """
        if self.buddycastfac is not None:
            self.buddycastfac.dialback_says_peer_connectable(permid) 
    ...
    def olthread_process_dialback_reply(self,dns,message):
        ...
        #print >>sys.stderr,"dialback: *********************************************** Dialback feature said reachable"
    ...
    def network_btengine_reachable_callback(self):
        ...
        #print >>sys.stderr,"dialback: ###################################################################### INCOMING CONN"
