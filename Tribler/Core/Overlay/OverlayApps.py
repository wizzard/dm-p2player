...
class OverlayApps:
    ...
    def register(self, overlay_bridge, session, launchmany, config, requestPolicy):
        ...
        if config['buddycast']:
            ...
            self.buddycast.register(overlay_bridge, launchmany, 
                                    ...
                                    config['start_recommender'],config['buddycast_max_peers'],i_am_crawler,config_channelcast=config['channelcast'])
        ...
        if config['dialback']:
            ...
            self.dialback_handler.register(overlay_bridge, launchmany, launchmany.rawserver, config, self.buddycast)
    ...
    def handleMessage(self,permid,selversion,message):
        ...
        if message[0] in self.msg_handlers:
            ...
            msgname = message_map.get(message[0],None)
            if DEBUG:
                print >> sys.stderr, "olapps: No handler found for", getMessageName(message[0:2])
            
            if msgname is not None:
                if DEBUG:
                    print >> sys.stderr, "olapps: Is known message type",msgname,",however, keeping connection"
                return True
            else:
                return False
