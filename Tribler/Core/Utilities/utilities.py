...
"""
# Arno: only be defined in sqlitecachedb.py, may only used in SqlCacheDBHandler.py"
def bin2str(bin):
    # Full BASE64-encoded 
...
def validTorrentFile(metainfo):
    ...
    if "initial peers" in metainfo:
        ...
        for address in metainfo["initial peers"]:
            ...
            if not ((isinstance(address, tuple) or isinstance(address, list)) and len(address) == 2):
    ...
    # PREDEF SEEDS
    if "predef seeders" in metainfo:
        if not isinstance(metainfo["predef seeders"], list):
            raise ValueError("predef seeders not list, but %s" % type(metainfo["predef seeders"]))
        for address in metainfo["predef seeders"]:
            #if not (isinstance(address, tuple) and len(address) == 2):
            if not ((isinstance(address, tuple) or isinstance(address, list)) and len(address) == 2):
                raise ValueError("address not 2-item tuple, but %s" % type(address))
            if not isinstance(address[0], str):
