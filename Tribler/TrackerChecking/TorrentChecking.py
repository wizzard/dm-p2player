...
class TorrentChecking(Thread):
    ...
    def selectPolicy(self):
        # Arno, 2011-06-16: Fix policy to oldest
    ...
    def run(self):
        ...
        try:
            ...
            if self.infohash is None and torrent['ignored_times'] > 0:
                if DEBUG:
                    print >> sys.stderr, 'Torrent Checking: torrent: %s' % torrent
            ...
            if DEBUG:
                print >> sys.stderr, "Torrent Checking: Tracker Checking",torrent['info']['announce']
            trackerChecking(torrent)
            
            if LIBRARYNAME != "BaseLib":
                # Arno, 2011-06-15: DHT check disabled for P2P-Next
            ...
            if DEBUG:
                print >> sys.stderr, "Torrent Checking: Result of check:", kw
                print >> sys.stderr, ""

            if LIBRARYNAME == "BaseLib":
                # Arno, 2011-06-15: P2P-Next: Delete torrent if dead
                if torrent['status'] == "dead":
                    if self.db_thread:
                        print >>sys.stderr,"Torrent Checking: Fatal error: DBTHREAD DELEGATION NOT IMPLEMENTED!!!!!!!!!!!!!!!"
                        
                    # TorrentDBHandler is "intelligent": doesn't delete anything 
                    # if in MyPreferences. We keep this behaviour to ensure lots
                    # of info when determining similarity in recommendation.
                    #if MyPreferenceDBHandler.getInstance().hasMyPreference(torrent['infohash']):
                    #    MyPreferenceDBHandler.getInstance().deletePreference(torrent['infohash'])

                    deleted = TorrentDBHandler.getInstance().deleteTorrent(torrent['infohash'], delete_file=True, commit = True)
                    if deleted:
                        return
                    # else: make sure status gets updated to dead
