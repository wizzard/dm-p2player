...
#        self.secure_overlay.send(target_permid, data, self.voteCastSendCallback)
    ...
    def createVoteCastMessage(self):
        ...
        records = self.votecastdb.getMyRecentAndRandomVotes()
    ...
    def handleVoteCastMsg(self, sender_permid, data):
        ...
        print >> sys.stderr, "votecast: Processing VOTECAST msg from: ", show_permid_short(sender_permid), "; len data: ", len(data)
        
        mod_ids = Set()
        for mod_id, value in data.items():
            #ARNOMAAKME: efficient DB commit!
            self.votecastdb.addVote(mod_id,sender_permid,value['vote'],value['time_stamp'])
            mod_ids.add(mod_id)
