...
class InterruptSocket:
    ...
    def interrupt(self):
        try:
            ...
            # Arno, 2010-12-13: ULANC reports this sometimes throws exceptions
            print_exc()
...
#        self.check.write(s)
        ...
        # M405 STBSPEEDCONFIG: always try to write
        if self.socket_handler.socketwritealways:
            self.try_write()
        else:
            # Original behaviour
...
class SocketHandler:
    def __init__(self, timeout, ipv6_enable, socketwritealways, socketrcvbufsize, socketsndbufsize, readsize = 100000):
        ...
        self.udp_sockets = {}
        self.socketwritealways = socketwritealways
        self.socketrcvbufsize = socketrcvbufsize
        self.socketsndbufsize = socketsndbufsize
    ...
    def start_connection_raw(self, dns, socktype = socket.AF_INET, handler = None):
        ...
        sock.setblocking(0)
        # M405 STBSPEEDCONFIG: increase from 87380 and 16384 (STB)
        if self.socketrcvbufsize is not None:
            sock.setsockopt(socket.SOL_SOCKET,socket.SO_RCVBUF,self.socketrcvbufsize)
        if self.socketsndbufsize is not None:
            sock.setsockopt(socket.SOL_SOCKET,socket.SO_SNDBUF,self.socketsndbufsize)
                ...
                if event & (POLLHUP | POLLERR) != 0:
                    ...
                    try:
                        ...
                        if len(self.single_sockets) < self.max_connects:
                            ...
                            # M405 STBSPEEDCONFIG: increase from 87380 and 16384 (STB)
                            if self.socketrcvbufsize is not None:
                                newsock.setsockopt(socket.SOL_SOCKET,socket.SO_RCVBUF,self.socketrcvbufsize)
                            if self.socketsndbufsize is not None:
                                newsock.setsockopt(socket.SOL_SOCKET,socket.SO_SNDBUF,self.socketsndbufsize)
