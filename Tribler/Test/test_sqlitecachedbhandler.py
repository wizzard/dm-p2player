...
from Tribler.Core.CacheDB.SqliteCacheDBHandler import TorrentDBHandler, MyPreferenceDBHandler, MyDBHandler, BasicDBHandler, PeerDBHandler, PreferenceDBHandler, SuperPeerDBHandler, FriendDBHandler, PopularityDBHandler, VoteCastDBHandler
...
class TestSqliteBasicDBHandler(unittest.TestCase):
    ...
    def singtest_getOne(self):
        ...
        # Arno, 2011-02-24: added 0 for v7+ scheme
...
class FakeSession:
    def __init__(self,my_permid):
        self.my_permid = my_permid
        
    def get_permid(self):
        return self.my_permid


class TestVoteCastDBHandler(unittest.TestCase):
    # Arno
    ...
    def setUp(self):
        ...
        self.my_permid = teststr2bin('MFIwEAYHKoZIzj0CAQYFK4EEABoDPgAEAAA6SYI4NHxwQ8P7P8QXgWAP+v8SaMVzF5+fSUHdAMrs6NvL5Epe1nCNSdlBHIjNjEiC5iiwSFZhRLsr')
        fakesession = FakeSession(self.my_permid)
        self.votecast_db = VoteCastDBHandler.getInstance()
        self.votecast_db.registerSession(fakesession)
        ...
        self.permid3 = teststr2bin("MFIwEAYHKoZIzj0CAQYFK4EEABoDPgAEAIV8h+eS+vQ+0uqZNv3MYYTLo5s0JP+cmkvJ7U4JAHhfRv1wCqZSKIuY7Q+3ESezhRnnmmX4pbOVhKTY")
        self.permid4 = teststr2bin("MFIwEAYHKoZIzj0CAQYFK4EEABoDPgAEAIV8h+eS+vQ+0uqZNv3MYYTLo5s0JP+cmkvJ7U4JAHhfRv1wCqZSKIuY7Q+3ESezhRnnmmX4pbOVhKTZ")
         
    def tearDown(self):
        SQLiteCacheDB.getInstance().close()

    def add_some_votes(self):
        self.votecast_db.subscribe(self.permid1)
        self.votecast_db.subscribe(self.permid2)

        self.votecast_db.addVote(self.permid1,self.permid3,1,132314)
        self.votecast_db.addVote(self.permid1,self.permid4,2,132315)

    def votetup2dict(self,votetup):
        return {'mod_id':votetup[0], 'voter_id':votetup[1],'vote':votetup[2], 'time_stamp':votetup[3]}

    # addVote() tested by getAllVotes
    def singtest_getVote(self):
        self.add_some_votes()
        
        vote = self.votecast_db.getVote(self.permid1,self.permid4)
        self.assertEqual(vote,2)

    def singtest_deleteVote(self):
        self.add_some_votes()
        
        self.votecast_db.deleteVote(self.permid1,self.permid4)
        votetup = self.votecast_db.getVote(self.permid1,self.permid4)
        self.assertTrue(voteup is None) 

    def singtest_deleteVotes(self):
        self.add_some_votes()
        
        self.votecast_db.deleteVotes(self.permid1)
        votetup = self.votecast_db.getVote(self.permid1,self.permid3)
        self.assertTrue(voteup is None) 
        votetup = self.votecast_db.getVote(self.permid1,self.permid4)
        self.assertTrue(voteup is None) 
        subs = self.votecast_db.getMySubscriptions()
        self.assertTrue(self.permid1 not in subs)

    
    def singtest_getVotes(self):
        self.add_some_votes()
        
        votetups = self.votecast_db.getVotes(self.permid1)
        self.assertEqual(len(votetups),3)
        for votetup in votetups:
            vote = self.votetup2dict(votetup)
            # Subscribe
            if vote['voter_id'] == self.my_permid:
                self.assertEqual(vote['mod_id'],self.permid1)
                self.assertEqual(vote['vote'],2)
            # Ext votes
            if vote['voter_id'] == self.permid3:
                self.assertEqual(vote['mod_id'],self.permid1)
                self.assertEqual(vote['vote'],1)
            if vote['voter_id'] == self.permid4:
                self.assertEqual(vote['mod_id'],self.permid1)
                self.assertEqual(vote['vote'],2)

    def singtest_getAllVotes(self):
        self.add_some_votes()
        
        votetups = self.votecast_db.getAllVotes()
        for votetup in votetups:
            vote = self.votetup2dict(votetup)
            # Subscribes
            if vote['voter_id'] == self.my_permid:
                self.assertTrue((vote['mod_id'] == self.permid1) or (vote['mod_id'] == self.permid2))
                self.assertEqual(vote['vote'],2)
    ...
    def singtest_getMyRecentAndRandomVotes(self):
        self.add_some_votes()

        tuples = self.votecast_db.getMyRecentAndRandomVotes()
        # [(mod_id,vote,timestamp)]
        favs = []
        for tuple in tuples:
            self.assertEqual(len(tuple),3)
            self.assertEqual(type(tuple[0]),type(""))
            self.assertEqual(type(tuple[1]),type(1))
            self.assertEqual(type(tuple[2]),type(1))
            favs.append(tuple[0])
            
        subs = self.votecast_db.getMySubscriptions()
        self.assertEqual(Set(favs),Set(subs))
    
    
    # subscribe() tested by getAllVotes
    def singtest_unsubscribe(self):
        self.add_some_votes()
        
        self.votecast_db.unsubscribe(self.permid2)
        votetup = self.votecast_db.getVote(self.permid2,self.my_permid)
        self.assertTrue(votetup is None) 
        subs = self.votecast_db.getMySubscriptions()
        self.assertTrue(self.permid2 not in subs)
        
    def singtest_getMySubscriptions(self):
        self.add_some_votes()
        
        subs = self.votecast_db.getMySubscriptions()
        self.assertTrue(self.permid1 in subs)
        self.assertTrue(self.permid2 in subs)
        self.assertTrue(len(subs) == 2)
        
    def singtest_spam(self):
        self.add_some_votes()
        
        self.votecast_db.spam(self.permid1)
        vote = self.votecast_db.getVote(self.permid1,self.my_permid)
        self.assertEqual(vote,-1)
        
        
    def singtest_getModIdsWithNegVote(self):
        self.add_some_votes()
        
        self.votecast_db.spam(self.permid1)
        spammers = self.votecast_db.getModIdsWithNegVote(self.my_permid)
        self.assertTrue(self.permid1 in spammers)
        
    def singtest_getNumNegVotes(self):
        self.add_some_votes()
        
        self.votecast_db.addVote(self.my_permid,self.permid3,-1,100)
        self.votecast_db.addVote(self.my_permid,self.permid4,-1,100)
        nhaters = self.votecast_db.getNumNegVotes(self.my_permid)
        self.assertTrue(nhaters == 2)
        
    def singtest_getNumSubscriptions(self):
        self.add_some_votes()
        
        nsubs = self.votecast_db.getNumSubscriptions(self.permid1)
        self.assertEqual(nsubs,2)
...
def test_suite():
    ...
    if len(sys.argv) != 3:
        print "Usage: python test_sqlitecachedbhandler.py <class name> <method name>"
    else:
