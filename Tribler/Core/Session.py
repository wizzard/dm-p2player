...
class Session(SessionRuntimeConfig):
    ...
    def remove_download(self,d,removecontent=False):  
        ...
        if d.get_def().get_def_type() == "torrent":
            ...
            # SWIFTPROC
            self.lm.swift_remove(d,removecontent=removecontent)
    ...
    def open_dbhandler(self,subject):
        ...
        NTFY_NS_RICHMETADATA -> NSRichMetadataDBHandler
        </pre>
        """ 
        # Called by any thread
        self.sesslock.acquire()
        try:
            ...
            elif subject == NTFY_NS_RICHMETADATA:
                return self.lm.nsrichmetadata_db
            else:
    ...
    def query_connected_peers(self,query,usercallback,max_peers_to_query=None):
        ...
        At the moment we support four types of query, which are all queries for
        ...
        kw3" (type 2). In the future we plan to support full SQL queries.
        ...
        * 'channel_name': channel name as Unicode string (or '').
        ...
        For SIMPLE+METADATA queries there is are extra fields
        <pre>
        * 'metatype': MIME type of the metadata (e.g. application/x-ns-stream)
        * 'metadata': Bencoded contents of the .torrent file for app/x-ns-stream 
        </pre>
        ...
        * 'time_stamp': as integer
        ...
        Because a channel is a stream of torrents which only have one injecter, 
        we can use this to more efficiently exchange ChannelCast data. After 
        subscribing to a channel, a user has to throw away all data. Then by 
        requesting a 'timeframe' to another peer he/she will receive 50 
        infohashes per request. In the request a peer has to specify the 
        mindate, maxdate and ntorrents inbetween those two timestamps. Using 
        this data, a peer can check if the requesting peer has a correct 
        'timeframe' and then return 50 newer ChannelCast entries. If less than 
        50 are available a peer will append older items to the items before 
        mindate until he has reached the 50 entry limit. If a peer detects an 
        incorrect timeframe, he will reply with the number of torrents the 
        requesting peer had plus one. To use this functionality add
        "min max ntorrents" to the CHANNEL query string. 

        The fourth type of query are RICHMETA+METADATA queries in the query
        language specified in Tribler/Plugin/README-RICHMETADATA_SEARCH.txt. The
        query result is the same as for SIMPLE+METADATA.
    ...
    def set_user_query_handlers(self,queryprefix2handlercallbackdict):
        """ Set query handlers for a given query prefix. This allows users
        to define their own query prefixes, send these queries to the peers
        it is currently connected and handle the replies in "user"-space
        (currently query replies are handled directly in the Core). Queries
        must be in the format <user-defined prefix> <rest>, i.e., a space
        separating the prefix from the rest of the query.
        
        The queryprefix2handlercallback dictionary contains a map of a Unicode 
        string, the query prefix to a query-handler callback function. When a 
        query with such a prefix is received, the callback will be called with 
        the following arguments:
        
            handlercallback(permid,query,qid,hitscallback)
            
        The calling thread is a network-handling thread that should 
        not be used for a long time. To return a reply, the user should call
        the hitscallback argument (either directly from the handlercallback
        or later via another thread if query processing is expected to take
        a long time). The hitscallback expects the following arguments:
        
            hitscallback(permid,qid,exc,hits)
            
        qid is binary query identifier string. exc should be None, or an 
        exception (in which a reply will not be sent). hits can be any data 
        that can be bencoded and has a length but preferably a dictionary.
        
        Query prefixes may include "SIMPLE", "CHANNEL", etc. to override
        the default query handling. A copy of the queryprefix2handlercallbackdict
        is made.
        
        @param queryprefix2handlercallbackdict A dictionary adhering to the 
        above spec.
        """
        self.sesslock.acquire()
        try:
            if self.sessconfig['overlay']:
                ...
                rqmh = RemoteQueryMsgHandler.getInstance()
                rqmh.set_user_query_handlers(queryprefix2handlercallbackdict)
            else:
                raise OperationNotEnabledByConfigurationException("Overlay not enabled")
        finally:
            self.sesslock.release()
    ...
    def download_torrentfile_from_peer(self,permid,infohash,usercallback, prio = 0):
        ...
        The prio parameter assigns a priority to the request. 0 being highest.
        Less urgency should be assigned to prevent the Core from generating
        too many requests/s. All torrents requested with a prio of 1 or lower, 
        will additionally immediately schedule a magnetlink request (BEP 9). 
        The DHT is additionally used for all other torrents if the peer did not
        respond to the torrent request.
