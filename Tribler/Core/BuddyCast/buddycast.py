...
def validBuddyCastData(prefxchg, nmyprefs=50, nbuddies=10, npeers=10, nbuddyprefs=10, selversion=0):
    ...
    # ProxyService_
    if (selversion >= OLPROTO_VER_FIFTEENTH and LIBRARYNAME == "Tribler") or selversion >= OLPROTO_VER_SIXTEENTH:
        try:
...
class BuddyCastFactory:
    ...
    def register(self, overlay_bridge, launchmany, errorfunc, 
                 ...
                 max_peers=2500,amcrawler=False,config_channelcast=True):
        if self.registered:
            return
        ...
        self.config_channelcast = config_channelcast
    ...
    def olthread_register(self, start=True):
        ...
        if self.config_channelcast:
            ...
            print >>sys.stderr,"bc: OlThread Register: disabling ChannelCast"
            self.votecast_core = None
            self.channelcast_core = None
        ...
        if start:
            ...
            if self.channelcast_core is not None:
    ...
    def dialback_says_peer_connectable(self,permid):
        if self.buddycast_core:
            if debug:
                print >> sys.stderr, "bc: dialback_says_peer_connectable:",show_permid_short(permid)
            self.buddycast_core.removeFromConnList(permid)
            self.buddycast_core.addPeerToConnList(permid,connectable=1)
...
class BuddyCastCore:
    ...
    def work(self):
        ...
        try:
            ...
            if self.next_initiate > 0:
                # It replied some messages in the last rounds, so it doesn't initiate Buddycast
    ...
    def _bootstrap(self, number):
        ...
        for peer_id in recent_peers_ids:
            ...
            permid = self.data_handler.getPeerPermid(peer_id)
            # Arno, 2011-01-28: Some extra foolproof filtering.
            if permid != self.permid and permid not in self.send_block_list:
                #print >>sys.stderr,"CANDIDATE IS",show_permid_short(permid),show_permid_short(self.permid)
        ...
        # Arno, 2011-01-28: Changed to >=, i.e. keep trying until you find some
        # No need to wait 300 s to look in database?!
    ...
    def startBuddyCast(self, target_permid):
        ...
        if not self.isBlocked(target_permid, self.send_block_list):
            ...
            # Arno, 2011-01-28: reestablished. Why was this removed?!
    ...
    def createBuddyCastMessage(self, target_permid, selversion, target_ip=None, target_port=None):
        ...
        if (selversion >= OLPROTO_VER_FIFTEENTH and LIBRARYNAME == "Tribler") or selversion >= OLPROTO_VER_SIXTEENTH:            
            ...
            print >>sys.stderr,"bc: Sending BC for OL version", selversion
        #
        # _ProxyService
            ...
            if step == 2:
                ...
                if self.booted < 0:
                    ...
                    for p in self.connection_candidates:
                        buf += "bc: * cand:" + show_permid_short(p) + "\n"
