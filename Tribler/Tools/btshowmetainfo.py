...
for metainfo_name in argv[1:]:
    ...
    print "metainfo creation date",metainfo['creation date'],"(aka",datetime.datetime.fromtimestamp(metainfo['creation date']),")"
    ...
    if info.has_key('playtime'):
        print 'playtime(info): %s' % `info['playtime']`
    if metainfo.has_key('playtime'):
        print 'playtime(top).: %s' % `metainfo['playtime']`

    if info.has_key('ns-metadata'):
        print 'ns-metadata...: %s' % `info['ns-metadata']`
