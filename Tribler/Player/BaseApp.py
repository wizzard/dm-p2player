...
from Tribler.Policies.RateManager import *
...
from Tribler.Core.Statistics.Status.Status import get_status_holder, TRIALLOG, NSPCLOG

DEBUG = False


DISKSPACE_LIMIT = 5L * 1024L * 1024L * 1024L  # 5 GB
# Arno, 2011-09-15: STBSPEED
DEFAULT_MAX_UPLOAD_SEED_WHEN_SEEDING = 1024 # KB/s
MAXCONNECTIONS = 256

RATELIMITADSL = False
ADSLDOWNLOADSPEED = 300
...
class BaseApp(wx.App,InstanceConnectionHandler):
    ...
    def OnInitBase(self):
        ...
        # Arno, 2011-03-02: Upgrade existing config to use overlay for search    
        self.configure_session()    
        self.sconfig.set_install_dir(self.installdir) # for when installdir is changed (e.g. running from source)
        ...
        if RATELIMITADSL:
            ...
            self.ratelimiter.set_global_max_speed(DOWNLOAD,ADSLDOWNLOADSPEED)
    ...
    def start_download(self,tdef,dlfile,poa=None,supportedvodevents=None):
        ...
        # Arno: 2011-3-29: Added connection limit. In VODO trial I saw
        # 450+ connections and maxed out CPU.
        #
        if MAXCONNECTIONS is not None:
            dcfg.set_max_conns_to_initiate(MAXCONNECTIONS)
            dcfg.set_max_conns(MAXCONNECTIONS)
    ...
    def restart_other_downloads(self):
        ...
        # Arno, 2010-09-16: Changed to one that doesn't give cap to non-uploaders :-(
    ...
    def load_playerconfig(self,state_dir):
        ...
        try:
            ...
            # Arno, 2011-09-15: Upgrade hack for ULANC
            if NSPCLOG and int(self.playerconfig['total_max_upload_rate']-300) <= 0:
                print >>sys.stderr,"bg: Upgrading upload limit for LivingLab to",DEFAULT_MAX_UPLOAD_SEED_WHEN_SEEDING
                self.playerconfig['total_max_upload_rate'] = DEFAULT_MAX_UPLOAD_SEED_WHEN_SEEDING 
