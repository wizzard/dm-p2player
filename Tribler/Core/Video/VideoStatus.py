...
class VideoStatus:
    ...
    def __init__(self,piecelen,fileinfo,videoinfo,authparams,kvconfig):
        """
        ...
        if self.live_streaming:
            # M405 STBSPEEDCONFIG
            self.dropping = kvconfig['live_drop_pieces']
        else:
            self.dropping = False # just wait and produce flawless playback
            
        self.upload_while_prebuf = kvconfig['upload_while_prebuf']
        ...
        if 'vod_useroffset' in videoinfo and videoinfo['vod_useroffset'] > 0:
            absoffset = movie_offset + videoinfo['vod_useroffset']
            piece = absoffset / piecelen
            
            print >>sys.stderr,"vodstatus: User wants offset",videoinfo['vod_useroffset'],"movie starts at",movie_offset,"falls inside piece",piece
            self.playback_pos = piece
        else:
