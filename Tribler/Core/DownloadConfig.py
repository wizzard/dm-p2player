...
class DownloadConfigInterface:
    ...
    def set_video_event_callback(self,usercallback,dlmode=DLMODE_VOD,offset=0):
        ...
        The offset parameter should be used with care. It is usable only
        for transport streams (as starting at an offset other than 0
        means the header of the video container will not be downloaded), or when
        you download the initial prefix using HTTP.
        
        @param usercallback  A function with the above signature.
        @param dlmode        The download mode to start in (_VOD or _SVC)
        @param offset        The offset to start from inside the selected file (_VOD only).
        """
        self.dlconfig['mode'] = dlmode
        self.dlconfig['vod_usercallback'] = usercallback
        
        # Arno, 2011-01-25: Allow user to specify offset to start from
        self.dlconfig['vod_useroffset'] = offset
    ...
    def set_max_conns(self,nconns):
        """ Sets the maximum number of connections for this 
        Download.
        @param nconns A number of connections.
        """
    ...
    def set_poa(self, poa):
        """ Set ClosedSwarm's Proof-of-Access
        @param poa binary PoA
        """
        if poa:
    ...
    def get_poa(self):
        """ Return Proof-of-Access for ClosedSwarms or None if not set.
        @return binary
        """
    ...
    def get_epoa(self):
        ...
        if 'poa' in self.dlconfig:
            ...
            from base64 import decodestring
            print >> sys.stderr,"get_poa:",self.dlconfig['poa']
            # Maybe not done good. Check better whether we can call a credential EPOA if without Rules field.
            try:
                poa = ECS_ClosedSwarms.EPOA.deserialize(decodestring(self.dlconfig['poa']))
            except:
                poa = ClosedSwarm.POA.deserialize(decodestring(self.dlconfig['poa']))
            return poa
        return None
    ...
    def set_reqservice(self, reqservice):
        """ Set ClosedSwarm's Requested Service
        @param reqservice binary ReqService
        """
        if reqservice:
            from base64 import encodestring
            from Tribler.Core.BitTornado.bencode import bencode
            reqservice = bencode(reqservice)
            self.dlconfig['reqservice'] = encodestring(reqservice.replace("\n",""))
            import sys
            print >> sys.stderr,"ReqService is set:",self.dlconfig['reqservice']
        
    def get_reqservice(self):
        """ Return Requested Service for ClosedSwarms or None if not set.
        @return binary
        """
        if 'reqservice' in self.dlconfig:
            if not self.dlconfig['reqservice']:
                raise Exception("No ReqService specified")
            from Tribler.Core.ClosedSwarm import ECS_ClosedSwarms
            from base64 import decodestring
            from Tribler.Core.BitTornado.bencode import bdecode
            print >> sys.stderr,"get_reqservice:",self.dlconfig['reqservice']
            reqservice = bdecode(decodestring(self.dlconfig['reqservice']))
            return reqservice
        return None
    ...
    def set_max_ecs_peers(self, value):
        ...
        @param reqservice binary ReqService
        """
        self.dlconfig['max_ecs_peers'] = value
        
    def get_max_ecs_peers(self):
        """ Return Requested Service for ClosedSwarms or None if not set.
        @return binary
        """
        return self.dlconfig['max_ecs_peers']
    ## ECS
    ...
    def set_give_to_get(self,value):
        """ Enable Give-to-Get, or set to automatic selection.
        @param value G2G_OFF/G2G_ON/G2G_AUTO
        """
        self.dlconfig['use_g2g'] = value

    def get_give_to_get(self):
        """ Returns whether Give-to-Get is enabled.
        @return G2G_OFF/G2G_ON/G2G_AUTO
        """
        return self.dlconfig['use_g2g']

    # M405 STBSPEEDCONFIG
    def set_live_drop_pieces(self,value):
        """ Enable or disable dropping of pieces in live streaming
        @param value Boolean
        """
        self.dlconfig['live_drop_pieces'] = value

    def get_live_drop_pieces(self):
        """ Returns whether dropping for live is enabled.
        @return Boolean
        """
        return self.dlconfig['live_drop_pieces']


    def set_upload_while_prebuf(self,value):
        """ Enable or disable uploading while prebuffering
        @param value Boolean
        """
        self.dlconfig['upload_while_prebuf'] = value

    def get_upload_while_prebuf(self):
        """ Returns whether uploading while prebuffering is enabled.
        @return Boolean
        """
        return self.dlconfig['upload_while_prebuf']

    def set_ratelim_min_resched_time(self,value):
        """ Set minimal time between calls to rate limiter. Set to 0.05 for
        limited clients, 0.0 otherwise.
        @param value float
        """
        self.dlconfig['ratelim_min_resched_time'] = value

    def get_ratelim_min_resched_time(self):
        """ Returns minimal time between calls to rate limiter.
        @return float
        """
        return self.dlconfig['ratelim_min_resched_time']
...
def get_default_dest_dir():
    ...
    * For UNIX: 
    ...
    return os.path.join(uhome,unicode(LIBRARYNAME+'Downloads'))
