...
def inorder_wraparound(vs,piecedict,wantfunc):
    # wantfunc returns false if there are requests outstanding for the piece
    keys = piecedict.keys()[:]
    keys.sort()

    first, last = vs.download_range()
    if first > last:
        # wraparound, serve 7100,7101,7102, etc. before 0,1,2,3,4,5
        for p in keys:
            if p >= first:
                if wantfunc(p):
                    return p
...
# _2fastbt
    ...
    def check_outstanding_requests(self, downloads):
        ...
        for download in downloads:
            ...
            for piece_id, begin, length in download.active_requests:
                ...
                # high-priority pieces are eligible for
                # cancellation. Others are not. They will eventually be
                # eligible as they become important for playback.
    ...
    def next_new(self, haves, wantfunc, complete_first, helper_con, willrequest=True, connection=None):
        ...
        if vs.live_streaming:
            ...
            try:
                # STBSPEED M405
                # 1. prebuffer only from source, if avail
                if vs.prebuffering:
                    connectedtosource = False
                    for conn in self.peer_connections.keys():
                        if conn.is_live_source():
                            connectedtosource = True
                            break
                
                    if connection.is_live_source() or not connectedtosource:
                        # 2. In-order download for prebuffering
                        pick = inorder_wraparound(vs,self.peer_connections[connection]["interesting"],wantfunc)
                        if DEBUGPP:
                            print >>sys.stderr,"Piece PICKPRE",pick,connection.get_ip()
                        return pick
                    else:
                        # Don't ask peers in prebuf phase
                        return None

                # STBSPEED M405:
                # Safety: Go to in-order download if lagging behind source.
                # Otherwise there are too many pieces to choose from
                # with rarest first and we won't dl the ones we need.
                #
                # 3. Or when running on empty.
                #
                avail = self.transporter.available()
                
                if len(self.peer_connections[connection]["interesting"]) > 40 or (avail is not None and avail == 0 and connection.is_live_source()):
                    # In-order download
                    pick = inorder_wraparound(vs,self.peer_connections[connection]["interesting"],wantfunc)
                    if DEBUGPP:
                        print >>sys.stderr,"Piece SEQPICK",pick,connection.get_ip()
                    return pick

                # 3. Not prebuffering 
                # Select any interesting piece, rarest first
                pick = rarest_first( self.peer_connections[connection]["interesting"], self.numhaves, wantfunc )

                if self.transporter.LIVE_PEERS_BIAS and pick and vs.live_startpos is not None:
                    # LIVEPEERSBIAS
                    # Arno, 2011-02-14: Implement a prefer-peers-over-source
                    # policy, if not prebuffering.
                    if connection.is_live_source():
                        due = self.transporter.piece_due(pick)-time.time()
                        trustdl = self.transporter.LIVE_PEERS_BIAS_TRUST_TILL_SECS
                        if due > trustdl:
                            ...
                            # from source.
                            # 
                            npeers = len(self.peer_connections)
                            ...
                            if since_last_upload < self.transporter.LIVE_PEERS_BIAS_UPLOAD_IDLE_SECS:
                                # TODO: model as increase in dlprob
                                r += self.transporter.LIVE_PEERS_BIAS_FORWARDER_DLPROB_BONUS
                            #print >>sys.stderr,"vod: pp: r %.02f dlprob %.02f due %.02f lastup %.02f" % (r,dlprob,due,since_last_upload)
            
                            # STBSPEED M405: simple live-peers bias: 90% from source
                            if npeers == 1:
                                dlprob = 1.0
                            else:
                                dlprob = self.transporter.LIVE_PEERS_BIAS_DLPROB
                            #print >>sys.stderr,"pp: bias: r",r,"dlprob",dlprob
                            if r >= dlprob:
                                # Trust you will get it from peers, don't dl from source
                                return None

                if DEBUGPP:                        
                    print >>sys.stderr,"Piece PICK",pick,connection.get_ip()
                return pick 
        ...
        #
        # Video-on-demand piece picking below this
        #
        # 1. prebuffer only from default seeders, if avail
        # PREDEFSEEDS
        if vs.prebuffering:
            connectedtosource = False
            for conn in self.peer_connections.keys():
                if conn.is_vod_source():
                    connectedtosource = True
                    break
        
            if not connection.is_vod_source() and connectedtosource:
                # Don't ask peers in prebuf phase
                return None
        else:
            # 2. VODPEERSBIAS: Refrain from downloading from source, if enabled
            if self.transporter.VOD_PEERS_BIAS and connection.is_vod_source():
                # STBSPEED M405: simple VOD-peers bias: x% from source, 
                # if not prebuffering.
                #
                # TODO: reward forwarding? Complex as peers which are
                # further in time will have more to forward.
                # 
                nconns = len(self.peer_connections)
                if nconns == 1:
                    dlprob = 1.0
                else:
                    dlprob = self.transporter.VOD_PEERS_BIAS_DLPROB 
