...
class Choker:
    # Arno, 2011-02-15: removed offending parameter :-(
    ...
    def _rechoke(self):
        # STBSPEED M405
        # Arno, 2011-09-12: Don't upload to others when prebuffering
        if self.videostatus is not None and self.videostatus.upload_while_prebuf and self.videostatus.prebuffering:
            if DEBUG:
                print >>sys.stderr,"Choker: no sharing while prebuffering"
            return
        ...
        if maxuploads > 1:
            ...
            for c in self.connections:
                ...
                if self.seeding_manager is None or self.seeding_manager.is_conn_eligible(c):
                    ...
                    if self.done():
                        r = u.get_rate()
                        #print >>sys.stderr,"choker: I am seed, candidate",c.get_ip(),r
                    else:
                        d = c.get_download()
                        r = d.get_rate()
                        #print >>sys.stderr,"choker: I am not seed, candidate",c.get_ip(),r
            ...
            if DEBUG:
                print >>sys.stderr,"choker: _rechoke: NORMAL UNCHOKE",[(x[0],x[1].get_ip()) for x in preferred]
            ...
            if DEBUG:
                print  >>sys.stderr,"choker: _rechoke: G2G UNCHOKE",[(x[0],x[1],x[2].get_ip()) for x in g2g_preferred]
                ...
                if count < maxuploads or not hit:
                    if self.seeding_manager is None or self.seeding_manager.is_conn_eligible(c):
                        if u.is_interested():
                            to_unchoke.append(u)
                            count += 1
                            if DEBUG and not hit: print  >>sys.stderr,"choker: OPTIMISTIC UNCHOKE",c.get_ip()
                            hit = True
    ...
    def interested(self, connection):
        # Arno, 2011-09-12: Ancient bug: we should reevaluate choking when
        # a new INTEREST comes in. Gave ~5/2 (5 = self.schedule time for
        # round robin) startup delay.
        #
