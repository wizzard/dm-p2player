...
class DownloadImpl:
    ...
    def create_engine_wrapper(self,lmcreatedcallback,pstate,lmvodeventcallback,initialdlstatus=None):
        ...
        if self.dlconfig['mode'] == DLMODE_VOD or self.dlconfig['video_source']:
            ...
            vodfileindex['vod_useroffset'] = self.dlconfig['vod_useroffset']
    ...
    def network_create_engine_wrapper(self,infohash,metainfo,kvconfig,multihandler,listenport,vapath,vodfileindex,lmcreatedcallback,pstate,lmvodeventcallback):
        ...
        try:
            ...
            if lmcreatedcallback is not None:
                lmcreatedcallback(self,sd,exc,pstate)
        except:
            print_exc()
        finally:
            self.dllock.release()
    ...
    def network_checkpoint(self):
        ...
        try:
            pstate = self.network_get_persistent_state() 
            if self.sd is None:
                # Arno,2011-02-07: If we're already stopped, use pstate_for_restart 
                # to get engineresumedata
    ...
    def recontact_tracker(self):
        ...
        self.dllock.acquire()
        try:
            self.session.lm.rawserver.add_task(self.network_recontact_tracker,0.0)
        finally:
            self.dllock.release()

    def network_recontact_tracker(self):
        self.dllock.acquire()
        try:
            if self.sd is not None:
                self.sd.recontact_tracker()
        finally:
            self.dllock.release()
