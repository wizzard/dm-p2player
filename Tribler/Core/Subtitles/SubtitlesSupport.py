...
from Tribler.Core.Utilities.utilities import isValidPermid, show_permid
import sys
import threading
...
class SubtitlesSupport(object):
    ...
    def publishSubtitle(self, infohash, lang, pathToSrtSubtitle):
        ...
        channelid = self.my_permid
        base64infohash = show_permid(infohash)
        # consistency check: I want to assure that this method is called
        # for an item that is actually in my channel
        consinstent = self.channelcast_db.isItemInChannel(channelid,infohash)
