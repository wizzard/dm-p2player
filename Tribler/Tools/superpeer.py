...
def olthread_start_session():
    ...
    # Arno, 2011-01-27: Enabled dialback. By acting as dialback server
    # superpeer now also records that a peer is connectable, so it learns
    # faster and can hand out a peer's contact address to others sooner
    # (not after next visit 4 hrs later, or via different superpeer)
    #
    sscfg.set_dialback(True)
    sscfg.set_dialback_active(False)
