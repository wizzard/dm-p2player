...
class MovieTransportStreamWrapper:
    ...
    def __init__(self,mt,useroffset=0):
        self.mt = mt
    ...
    def read(self,numbytes=None):
        ...
        if not self.started:
            if DEBUG:
                print >>sys.stderr,"MovieTransportStreamWrapper: read: not started, user offset is",self.useroffset
                
            self.mt.start(self.useroffset)
            self.started = True
        if self.mt.done():
            return ''
