...
def calc_hash_slot(peer_id,nslots):
...
class Tracker:
    ...
    def get_infopage(self):
        try:
            ...
            s.write('<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.1//EN" "http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd">\n' \
                '<html><head><title>Tracker Statistics</title>\n')
    ...
    def peerlist(self, infohash, stopped, tracker, is_seed, return_type, rsize, peer_id):
        data = {}    # return data
        ...
        # PREDEFSEED
        if 'tracker_send_predefseeds' in self.config and self.config['tracker_send_predefseeds'] == True:

            if DEBUG:
                print >>sys.stderr,"track: predefseed: Before peerlist is",`peerdata`,"is_seed",is_seed
            
            if not is_seed and 'predefseed' in self.state['allowed'][infohash]:
                aliveseeds = []
                for ipport in self.state['allowed'][infohash]['predefseed']:
                    for peerid in self.downloads[infohash]:
                        peerrec = self.downloads[infohash][peerid]
                        if peerrec['ip'] == ipport[0] and peerrec['port'] == ipport[1]:
                            # Found an alive predef seed
                            aliveseeds.append(ipport)
                            if DEBUG:
                                print >>sys.stderr,"track: predefseed: Found alive seed",ipport
                            
                slot = calc_hash_slot(peer_id,len(aliveseeds))
                if slot is not None:
                    if DEBUG:
                        print >>sys.stderr,"track: predefseed: Slot for",`peer_id`,"is",slot,"seed",aliveseeds[slot]
                    peerdata.append(compact_peer_info(aliveseeds[slot][0],aliveseeds[slot][1]))
                    
            if DEBUG:
                print >>sys.stderr,"track: predefseed: After peerlist is",`peerdata`,return_type
    ...
    def get(self, connection, path, headers):
        ...
        data = self.peerlist(infohash, event=='stopped',
                             ...
                             return_type, rsize, params('peer_id'))
