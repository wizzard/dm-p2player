...
from Tribler.Core.Statistics.Status.Status import TRIALLOG

try:
    # ECS
    from base64 import decodestring
    from Tribler.Core.ClosedSwarm import ClosedSwarm, ECS_ClosedSwarms
    from Tribler.Core.ClosedSwarm.ECS_AuthorizationEngine import get_max_reqservice
except:
    print_exc()
...
# ECS: protocol versions
CLOSED_SWARMS = 1
ENHANCED_CLOSED_SWARMS = 2
...
class Connection:
    def __init__(self, connection, connecter):
        ...
        # ECS: this attribute contains ECS_Connection object reference
        self.ecsconnection = None
        ...
        if self.connecter.is_closed_swarm:
            ...
            # ECS: CS protocol gets POA
            if self.connecter.cs_version == CLOSED_SWARMS:
    ...
    # ECS
    def set_ecsconnection(self, ecscon):
        self.ecsconnection = ecscon
    ...
    def close(self):
        ...
        # ECS
        if self.ecsconnection is not None:
            self.ecsconnection.ecs_swarm_manager.unregister_connection(self)
    ...
    def send_interested(self):
        # ECS: start protocol
        if self.ecsconnection is None:
            self._send_message(INTERESTED)
        else:
            if self.ecsconnection.is_dl_ecs_completed():
                self._send_message(INTERESTED)
            else:
                self.ecsconnection.start_ecs()
    ...
    def send_partial(self, bytes):
        ...
        if self.partial_message is None:
            ...
            # ECS: encrypt piece
            if self.is_closed_swarm and self.connecter.cs_version == ENHANCED_CLOSED_SWARMS:
                if self.ecsconnection.is_ul_ecs_completed():
                    from array import array
                    key, iv = self.ecsconnection.get_ul_key_iv()
                    enc_piece = ECS_ClosedSwarms.encrypt_AES(piece.tostring(), key, iv) 
                    piece = array('c', enc_piece)
    ...
    def got_extend_handshake(self,d):
        ...
        if 'm' in d:
            ...
            if EXTEND_MSG_CS in self.extend_hs_dict['m']:
                ...
                # ECS
                if self.is_closed_swarm:
                    if self.connecter.cs_version == CLOSED_SWARMS:
                        if self.connection.locally_initiated:
                            if DEBUG_CS:
                                print >>sys.stderr,"connecter: Initiating Closed swarm handshake"
                            self.start_cs_handshake()
                    elif self.connecter.cs_version == ENHANCED_CLOSED_SWARMS:
                        ecsmanager = ECS_ClosedSwarms.ECS_Manager.getInstance()
                        ecs_swarm_manager = ecsmanager.get_swarm_manager(self.connecter.infohash)
                        self.ecsconnection = ecs_swarm_manager.register_connection(self)
                        ecs_swarm_manager.set_connections_starter(self.connection.Encoder.start_connections)
                        if self.connection.locally_initiated:
                            cs_message = self.ecsconnection.start_ecs()
                            self._send_cs_message(cs_message)
            else:
                self.remote_supports_cs = False
                if TRIALLOG:
    ...
    def got_cs_message(self, cs_list):
        ...
        # ECS
            ...
            response = self.ecsconnection.got_cs_message(cs_list)
            if response is not None and not isinstance(response, bool):
                self._send_cs_message(response)
                if self.ecsconnection.terminate_after_sending(response):
                    self.ecsconnection.terminate()
            if self.connection.locally_initiated:
                if self.ecsconnection.is_dl_ecs_completed():
                    self.cs_complete = True
                elif self.ecsconnection.is_ul_ecs_completed():
                    self.connecter.cs_handshake_completed()
            elif self.ecsconnection.is_ul_ecs_completed():
                self.connecter.cs_handshake_completed()
                self.cs_complete = True
            elif self.ecsconnection.is_dl_ecs_completed():
    ...
    def got_g2g_piece_xfer_v1(self,index,begin,length):
        ...
        print >>sys.stderr,"GOT G2G v1",index,begin,length
    ...
    def got_g2g_piece_xfer_v2(self,ppdict):
        ...
        print >>sys.stderr,"GOT G2G v2 #",len(ppdict)
    ...
    def is_live_source(self):
        if self.connecter.live_streaming:
            if self.get_ip() == self.connecter.tracker_ip:
                return True
            # PREDEFSEEDS
            for ipport in self.connecter.predef_seeders:
                if self.get_ip() == ipport[0]:
                    return True
        return False

    def is_vod_source(self):
        # For testing
        #return self.download.peer_is_complete()
        if not self.connecter.live_streaming:
            # PREDEFSEEDS
            for ipport in self.connecter.predef_seeders:
                if self.get_ip() == ipport[0]:
                    return True
...
# 2fastbt_
    def __init__(self, metadata, make_upload, downloader, choker, numpieces, piece_size,
            ...
            coordinator = None, helper = None, get_extip_func = lambda: None, mylistenport = None, use_g2g = False, infohash=None, tracker=None, live_streaming = False, predef_seeders = []):
        ...
        self.predef_seeders = predef_seeders
        ...
        if 'cs_keys' in self.config:
            if self.config['cs_keys'] != None:
                if len(self.config['cs_keys']) == 0:
                    ...
                    # ECS: set POA, ReqService and scheduler in swarm manager responsible
                    ecsmanager = ECS_ClosedSwarms.ECS_Manager.getInstance()
                    ecs_swarm_manager = ecsmanager.get_swarm_manager(self.infohash)
                    if 'max_ecs_peers' in self.config:
                        ecs_swarm_manager.set_max_ecs_peers(self.config['max_ecs_peers'])
                    if 'poa' in self.config:
                        try:
                            poa = ECS_ClosedSwarms.EPOA.deserialize(decodestring(self.config['poa']))
                            ecs_swarm_manager.set_poa(poa)
                        except Exception,e:
                            print_exc()
                        if DEBUG_CS:
                            print >> sys.stderr, "ECS: Poa set to the ECS Connections (Swarm) Manager"
                    if 'reqservice' in self.config:
                        try:
                            reqservice = bdecode(decodestring(self.config['reqservice']))
                            ecs_swarm_manager.set_reqservice(reqservice)
                        except Exception,e:
                            print_exc()
                        if DEBUG_CS:
                            print >> sys.stderr, "ECS: ReqService set to the ECS Connections (Swarm) Manager"
                    else:
                        reqservice = get_max_reqservice(poa.rules)
                        ecs_swarm_manager.set_reqservice(reqservice)

                    # Maybe raise exceptions if not??

                    ecs_swarm_manager.set_scheduler(self.sched)

                    # ECS: set cs_version field based on the number of fields in POA (EPOA has 6)
                    if len(ecs_swarm_manager.poa.serialize_to_list()) == 6:
                        self.cs_version = ENHANCED_CLOSED_SWARMS
                    else:
                        self.cs_version = CLOSED_SWARMS
    ...
    def connection_lost(self, connection):
        ...
        # ECS: close connection when lost
        if not c.closed:
            c.close()
    ...
    def got_hashpiece(self, connection, message):
        ...
        try:
            ...
            # Arno, 2011-02-11: Not need to handle live
    ...
    def got_message(self, connection, message):
        ...
        if t == CHOKE:
            ...
            try:
                # ECS
                piece = message[9:]
                if self.is_closed_swarm and self.cs_version == ENHANCED_CLOSED_SWARMS:
                    if c.ecsconnection.is_dl_ecs_completed():
                        key, iv = c.ecsconnection.get_dl_key_iv()
                        piece = ECS_ClosedSwarms.decrypt_AES(piece, key, iv)
                if c.download.got_piece(i, toint(message[5:9]), [], piece):
                    self.got_piece(i)
            except ValueError,ve:
                if c.is_live_source():
                    print >>sys.stderr,"connecter: Got PIECE from live source that failed check, but keeping connection"
                    print_exc()
                else:
                    connection.close()
                    return
    ...
    def close_all(self):
        for cc in self.connections.values():
            cc.close()


    def limit_connections(self):
        """ Impose max_connections limit by closing conns witht least dl/ul """
        limit = self.config['max_connections']
        newconns = sorted(self.connections.values(),cmp=sort_conn_on_speeds_desc)
        byeconns = newconns[limit:]
        for cc in byeconns:
            cc.close()
...
def sort_conn_on_speeds_desc(a,b):
    """ Sort Connection objects based on current ul/dl speeds, descending
    """
    aspeed = a.download.measure.get_rate_noupdate() + a.upload.measure.get_rate_noupdate()
    bspeed = b.download.measure.get_rate_noupdate() + b.upload.measure.get_rate_noupdate()
    if aspeed < bspeed:
        return 1
    elif aspeed > bspeed:
        return -1
