...
class TestRemoteQuery(TestAsServer):
    ...
    def setUpPostSession(self):
        ...
        try:
            ...
            self.torrent_db.addExternalTorrent(tdef3, extra_info={"filename":"sumfilename3",'status':'good'})
            ...
            dbrec= self.torrent_db.addExternalTorrent(tdef1, extra_info={"filename":"usumfilename1",'status':'good'})
            
            tdef2, bmetainfo2 = self.get_default_torrent('usumfilename2',u'Ch\u00e8rie S02E01')
            dbrec = self.torrent_db.addExternalTorrent(tdef2, extra_info={"filename":"usumfilename2",'status':'good'})
    
            tdef3, bmetainfo3 = self.get_default_torrent('usumfilename3',u'Cherie Demo')
            self.torrent_db.addExternalTorrent(tdef3, extra_info={"filename":"usumfilename3",'status':'good'})
            ...
            dbrec= self.torrent_db.addExternalTorrent(tdef1, extra_info={"filename":"psumfilename1",'status':'good'})
            
            paths2 = ['SomeFile.mkv',u'Besan\u00e7on.doc']
            tdef2, bmetainfo2 = self.get_default_torrent('psumfilename2',u'Path S02E01',paths=paths2)
            dbrec = self.torrent_db.addExternalTorrent(tdef2, extra_info={"filename":"psumfilename2",'status':'good'})
    
            paths3 = ['SomeFile.mkv',u'Besancon']
            tdef3, bmetainfo3 = self.get_default_torrent('psumfilename3',u'Path Demo',paths=paths3)
            self.torrent_db.addExternalTorrent(tdef2, extra_info={"filename":"psumfilename3",'status':'good'})
            ...
            dbrec= self.torrent_db.addExternalTorrent(tdef1, extra_info={"filename":"ssumfilename1",'status':'good'})
            
            tdef2, bmetainfo2 = self.get_default_torrent('ssumfilename2','Two S02E01 One')
            dbrec = self.torrent_db.addExternalTorrent(tdef2, extra_info={"filename":"ssumfilename2",'status':'good'})
    
            tdef3, bmetainfo3 = self.get_default_torrent('ssumfilename3','Two Demo')
            self.torrent_db.addExternalTorrent(tdef3, extra_info={"filename":"ssumfilename3",'status':'good'})
    ...
    def test_all(self):
        ...
        time.sleep(5)
        self.subtest_good_richmetaplusmetadata_query("title~hallo",self.goodtorrents_str)
        time.sleep(5)
    ...
    def subtest_good_richmetaplusmetadata_query(self,keyword,goodtorrents):
        """ 
            test good QUERY messages: RICHMETA+METADATA
        """
        print >>sys.stderr,"test: good QUERY RICH+METADATA",`keyword`
        s = OLConnection(self.my_keypair,'localhost',self.hisport)
        msg = self.create_good_richmetaplusmetadata_query(keyword)
        s.send(msg)
        resp = s.recv()
        if len(resp) > 0:
            print >>sys.stderr,"test: good QUERY: got",getMessageName(resp[0])
        self.assert_(resp[0] == QUERY_REPLY)
        self.check_rquery_reply("RICHMETA+METADATA",resp[1:],goodtorrents)
        time.sleep(10)
        # the other side should not have closed the connection, as
        # this is all valid, so this should not throw an exception:
        s.send('bla')
    ...
    def create_good_richmetaplusmetadata_query(self,keyword):
        d = {}
        d['q'] = 'RICHMETA+METADATA '+keyword.encode("UTF-8")
        d['id'] = 'b' * 20
        return self.create_payload(d)
