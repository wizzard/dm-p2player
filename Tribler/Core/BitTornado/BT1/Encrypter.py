...
# _2fastbt        
    ...
    def _start_connection_from_queue(self,sched=True):
        try:
            ...
            if DEBUG:
                print >>sys.stderr,"encoder: conns",cons,"max conns",self.get_max_connections(),"max init",max_initiate
            
            if cons >= self.get_max_connections() or cons >= max_initiate:
                delay = 60.0
    ...
    def start_connection(self, dns, id, coord_con = False, proxy_con = False, forcenew = False, challenge = None, proxy_permid = None):
        ...
        if ( self.paused
             or len(self.connections) >= self.get_max_connections()
             or id == self.my_id
    ...
    def external_connection_made(self, connection):
        ...
        if self.paused or len(self.connections) >= self.get_max_connections():
...
# _2fastbt    
    ...
    def admin_close(self,conn):
        ...
        # Arno, 2011-05-09: Disabled, appears to cause loops and memory leaks
        """
        now = time()
